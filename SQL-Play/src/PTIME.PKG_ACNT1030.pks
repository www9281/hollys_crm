CREATE OR REPLACE PACKAGE       PKG_ACNT1030 AS

    ---------------------------------------------------------------------------------------------------
    --  Package Name     : PKG_ACNT1030
    --  Description      : 매장 전도금 입금 확정    
    -- Ref. Table        :
    ---------------------------------------------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY       PKG_ACNT1030 AS
    ...
    BEGIN
        ...
        ELSIF( PSV_TP = 'C'  ) THEN
             ...
             IF( v_confirm_yn = 'Y' ) THEN
                ...
                IF(TO_DATE(PSV_PRC_DT, 'YYYYMMDD') < TO_DATE(TO_CHAR( LAST_DAY(ADD_MONTHS (SYSDATE , -3)) +1 , 'YYYYMMDD'), 'YYYYMMDD')) THEN 
                
                    ls_err_cd   := '-1' ;
                    ls_err_msg  := '취소 처리는 2개월이내 데이터까지만 가능합니다.';      
