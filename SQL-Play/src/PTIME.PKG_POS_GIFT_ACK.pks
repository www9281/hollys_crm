CREATE OR REPLACE PACKAGE       PKG_POS_GIFT_ACK AS
  ------------------------------------------------------------------------------
  --  Package Name     : PKG_POS_GIFT_ACK
...
END PKG_POS_GIFT_ACK;

/

CREATE OR REPLACE PACKAGE BODY       PKG_POS_GIFT_ACK AS
  ...
  BEGIN
    ...
    VALUES
           (ps_in_comp_cd   ,
            ...
            li_cnt2         ,
            li_cnt3         ,
            '2'             , --판매종류[00440> 0:상품판매, 1:회원권판매, 2:상품권판매, 3:상품권거스름, 99:폐기]
            'Y'             ,
            SYSDATE         ,
            ps_in_req_user  ,
            SYSDATE         ,
  ...
  (
    ...
    lsv_stor_cd     GIFT_MST.STOR_CD%TYPE;
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT MAX(CASE WHEN C.CODE_CD = '0000' THEN NVL(L.CODE_NM, C.CODE_NM) ELSE NULL END)  MSG0,
             ...
             MAX(CASE WHEN C.CODE_CD = '0035' THEN NVL(L.CODE_NM, C.CODE_NM) ELSE NULL END)  MSG35
        INTO lsv_msg0,  lsv_msg1,  lsv_msg2,  lsv_msg3,  lsv_msg4,
             ...
             lsv_msg23, lsv_msg31, lsv_msg32, lsv_msg33, lsv_msg34, lsv_msg35
        FROM COMMON C
           , (SELECT COMP_CD
                   , CODE_CD
                   , CODE_NM
    ...
    BEGIN
      BEGIN
         SELECT A.GIFT_STAT_CD, A.S_APPR_NO   , A.GIFT_CD    , A.PRICE      , A.BRAND_CD  , A.STOR_CD    , B.CODE_CD    , NVL(LC.CODE_NM, B.CODE_NM), A.EXP_DT,
                A.COMP_CD     , A.S_BRAND_CD  , A.S_STOR_CD  , A.S_SALE_DT  , A.S_POS_NO  , A.S_BILL_NO  , NVL(LS.STOR_NM, D.STOR_NM), E.BRAND_CD,
                GL.GIFT_STAT_CD
           INTO lsv_gift_stat , lsv_app_seq   , lsv_gv_cd    , lnv_price    , lsv_brand_cd, lsv_stor_cd  , lsv_code_cd  , lsv_CODE_NM               , lsv_exp_dt,
      ...
      IF ps_in_req_div IN ('20', '50') THEN -- 20:상품권 조회, 50:상품권 판매 조회
         ...
         ELSE
            ...
            ELSE
               IF ps_in_req_div = '20' AND lsv_gift_stat NOT IN ('20', '30') THEN -- 상품권 결제 가능 상태가 아닌 경우(점포고객판매)
                  lsv_msg_code   := '0020';
                  lsv_msg_text   := lsv_msg20; 
               ELSIF ps_in_req_div = '50' THEN
                  IF ps_in_sale_div = '0200' THEN
                     IF lsv_gift_stat IN ('20', '30') THEN        -- 상품권이 이미 판매된 경우
                        lsv_msg_code   := '0035';
                        lsv_msg_text   := lsv_msg35;
                     ELSIF lsv_gift_stat NOT IN ('11', '31') THEN -- 상품권 판매 가능 상태가 아닌 경우
                        lsv_msg_code   := '0020';
                        lsv_msg_text   := lsv_msg20;
                     ELSIF ps_in_stor_cd <> lsv_stor_cd THEN      -- 상품권 판매시 출고매장과 판매 매장이 다른 경우
                        lsv_msg_code   := '0005';
                        lsv_msg_text   := lsv_msg5;
                     ELSE
                        ...
                        END IF;
                      END IF;
                  ELSE                                            -- 상품권 판매 반품 가능여부 체크
                     IF lsv_gift_stat IN ('20', '30') THEN
                        IF ps_in_stor_cd <> lsv_stor_cd THEN      -- 상품권 반품시 판매 매장과 반품 매장이 다른 경우
                            lsv_msg_code   := '0005';
                            lsv_msg_text   := lsv_msg5;
                        ELSE
                            lsv_msg_code := '0000';
      ...
      ELSIF ps_in_req_div = '40' THEN -- 망상취소
         ...
         END;
      ELSIF ps_in_req_div IN ('10', '30') THEN -- 승인/상품권 판매
         CASE 
            WHEN lsv_gift_stat IS NULL  THEN
                lsv_msg_code   := '0001';
                lsv_msg_text   := lsv_msg1;
            ...
            WHEN lsv_gift_stat IN ('11', '12', '21', '31') THEN   -- 고객판매대기
                IF ps_in_req_div = '10' THEN                        -- 승인요청 
                    lsv_msg_code := '0020';
                    lsv_msg_text   := lsv_msg20;
                ELSE                                                -- 상품권판매요청
                    IF ps_in_stor_cd <> lsv_stor_cd THEN             -- 출고매장과 판매 매장이 다른 경우
                        lsv_msg_code   := '0005';
                        lsv_msg_text   := lsv_msg5;
                    ELSIF ps_in_sale_div != '0200' THEN
                        lsv_msg_code   := '0009';
                        lsv_msg_text   := lsv_msg9;
                    ELSE
                        lsv_msg_code   := '0000';
                        lsv_msg_text   := lsv_msg0;
                        lsv_gift_stat2 := '30';
            ...
            WHEN lsv_gift_stat IN ('50', '51') THEN   -- 사고등록
                lsv_msg_code := '0007';
                ...
                IF ps_in_req_div = '10' THEN
                    IF ps_in_sale_div = '0200' THEN
                        ...
                        IF lsv_out_msg2 IS NOT NULL THEN
                            lsv_out_msg3 := FC_GET_WORDPACK(ps_in_comp_cd, lsv_language, 'GIFT_MSG_01');
                        END IF;
                    ELSIF ( ps_in_brand_cd = lsv_o_brand_cd AND ps_in_stor_cd = lsv_o_stor_cd AND ps_in_req_proc = '2') OR lsv_app_seq = ps_in_o_bill_no THEN                              -- 정상 취소
                        IF ps_in_sale_div = '0420' THEN
                            lsv_msg_code   := '0000';
                            lsv_msg_text   := lsv_msg0;
                            lsv_gift_stat2 := '99'; -- 판매승인 후 영수증반품인 경우 폐기 처리하고 현금 지급함.
                        ELSIF ps_in_sale_div = '0430' THEN
                            lsv_msg_code   := '0000';
                            lsv_msg_text   := lsv_msg0;
                            lsv_gift_stat2 := '30'; -- 판매승인 후 반품(사용가능)인 경우 고객판매 상태로 환원
                        END IF;
                    ELSE
                        lsv_msg_code   := '0010';
            ...
            WHEN lsv_gift_stat IN ('12', '21', '31') THEN             -- 회수
                ...
                IF lsv_out_msg2 IS NOT NULL THEN
                   lsv_out_msg3 := FC_GET_WORDPACK(ps_in_comp_cd, lsv_language, 'GIFT_MSG_01');
                END IF;
            WHEN lsv_gift_stat IN ('99') THEN                         -- 폐기
                lsv_msg_code := '0023';
    ...
    BEGIN
      ...
      -- 10:승인(상품권결제)/30:상품권판매
      IF ps_in_req_div = '30' THEN
          UPDATE GIFT_MST
             SET GIFT_STAT_CD  = lsv_gift_stat2,
                 SC_SALE_DT    = DECODE(ps_in_sale_div, '0200', ps_in_sale_dt, SC_SALE_DT ),
                 SC_SALE_SEQ   = DECODE(ps_in_sale_div, '0200', ps_in_bill_no, SC_SALE_SEQ),
                 SC_RTN_DT     = DECODE(ps_in_sale_div, '0200', NULL         , ps_in_sale_dt),
                 SC_APPR_NO    = lsv_app_seq,
                 UPD_DT        = SYSDATE,
  ...
  (
    ...
    ps_in_req_proc   IN   VARCHAR2, -- Procees Channel ('1':Store , '2': SYSTEM , '3': Call Center , '4':On-Line Cancel at Process Error)
    ps_in_req_div    IN   VARCHAR2, -- Request Type (10:Approval  20:Retrieval  40:On-Line Cancel at Process Error)
    ps_in_sale_div   IN   VARCHAR2, -- Sale/Return(0200:Sale, 0420:Return(폐기처리), 0430:Return(판매가능))
    ps_in_o_sale_dt  IN   VARCHAR2, -- Sale Date
    ps_in_o_bill_no  IN   VARCHAR2, -- Bill No.
    ps_out_rtn_cd    OUT  VARCHAR2, -- Result Message
    ps_out_msg1      OUT  VARCHAR2, -- Message1
  ...
  BEGIN
    ...
    ( ps_in_gc_no      , -- 01 상품권번호
      ...
      ps_in_req_div    , -- 09 요청구분 (10:승인  20:조회 30:상품권판매 40:망상취소, 50:상품권판매조회)
      ps_in_sale_div   , -- 10 정상/반품(0200:정상, 0420:반품(폐기), 0430:반품(판매가능))
    ...
    IF ( lsv_msg_code <> '0000' ) THEN
       IF ( ps_in_sale_div = '0200' AND ps_in_req_div = '20' ) THEN -- 정상, 조회 요청시
           IF ( lsv_msg_code IN ( '9020', '9030' ) ) THEN
              ps_out_rtn_cd := '0000';
           END IF;
       ELSIF ( ps_in_sale_div = '0200' AND ps_in_req_div = '50' ) THEN -- 정상, 상품권판매조회
           IF ( lsv_msg_code IN ('9011', '9031') ) THEN
              ps_out_rtn_cd := '0000';
              ps_out_msg1 := FC_GET_WORDPACK_MSG(ps_in_comp_cd, lsv_language, '1010001559');
           ELSIF ( lsv_msg_code IN ( '9020', '9030' ) ) THEN
              ps_out_msg1 := FC_GET_WORDPACK_MSG(ps_in_comp_cd, lsv_language, '1010001560');
           END IF;
       END IF;
    END IF;
