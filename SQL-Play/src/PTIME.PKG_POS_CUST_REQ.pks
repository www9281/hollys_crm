CREATE OR REPLACE PACKAGE PKG_POS_CUST_REQ AS
--------------------------------------------------------------------------------
--  Package Name     : PKG_POS_CUST_REQ
--  Description      : POS 멤버십 요청 응답
--  Ref. Table       : 
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
  ...
  P_COMP_CD        VARCHAR2(4)  := '000'; -- DEFAULT
  ...
  (
    ...
    asRetVal              OUT  NUMBER  , -- 5. 결과코드[1:정상  그외는 오류]
  ...
  (
    ...
    PSV_STD_ROW           IN   NUMBER  , -- 6. 페이지당 레코드 수
    asRetVal              OUT  NUMBER  , -- 7. 결과코드[1:정상  그외는 오류]
  ...
  (
    ...
    PSV_CRG_FG            IN   VARCHAR2, -- 20. 결제구분[1:충전, 2:취소, 3:환불, 4:이전, 9:조정]
    PSV_CRG_DIV           IN   VARCHAR2, -- 21. 결제방법[1:현금, 2:신용카드, 9:조정]
    PSV_CRG_AMT           IN   NUMBER,   -- 22. 결제금액
  ...
  (
    ...
    PSV_USE_AMT           IN   NUMBER,   -- 7. 사용금액
    PSV_SAV_MLG           IN   NUMBER,   -- 8. 적립마일리지
    PSV_SAV_PT            IN   NUMBER,   -- 9. 적립포인트
    PSV_USE_PT            IN   NUMBER,   -- 10. 사용포인트
...
END PKG_POS_CUST_REQ;

/

CREATE OR REPLACE PACKAGE BODY      PKG_POS_CUST_REQ AS
--------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    OPEN asResult FOR
        SELECT  CRG_DT
             ...
             ,  CNC_DIV
             ,  ORG_CNC_DIV
             ,  CASE WHEN MOD(ROW_CNT, PSV_STD_ROW) = 0 THEN TRUNC(ROW_CNT / PSV_STD_ROW)
                     ELSE TRUNC(ROW_CNT / PSV_STD_ROW) + 1 
                END CUR_PAGE
             ,  CASE WHEN MOD(MAX_CNT, PSV_STD_ROW) = 0 THEN TRUNC(MAX_CNT / PSV_STD_ROW)
                     ELSE TRUNC(MAX_CNT / PSV_STD_ROW) + 1 
                END MAX_PAGE
        FROM   (     
                SELECT CRG_DT
                     , CRG_SEQ
        ...
        WHERE   PSV_PAGE_NO = (
                                CASE WHEN PSV_PAGE_NO = 'A' THEN 'A' 
                                     ELSE (
                                           CASE WHEN MOD(ROW_CNT, PSV_STD_ROW) = 0 THEN TO_CHAR(TRUNC(ROW_CNT / PSV_STD_ROW), 'FM999999')
                                                ELSE TO_CHAR(TRUNC(ROW_CNT / PSV_STD_ROW) + 1, 'FM999999')
                                           END
                                          ) 
                                END
                               );
...
--------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    ELSE
       ...
       BEGIN
         SELECT CP.PARA_VAL, BM.BRAND_CD, '0000000' 
           INTO lscard_div , lsbrand_cd , lsstor_cd
           FROM PARA_COMPANY CP
              , BRAND_MEMB   BM
          WHERE CP.COMP_CD  = BM.COMP_CD
            AND CP.PARA_CD  = '2003' -- 카드관리범위[1:회사, 2:영업조직, 3:점포]
            ...
            AND BM.USE_YN   = 'Y'; -- 사용여부[Y:사용, N:사용안함]
       EXCEPTION
         WHEN OTHERS THEN 
              asRetVal := '1002';
  ...
  BEGIN
    ...
    ELSE
       BEGIN
         SELECT CP.PARA_VAL, BM.BRAND_CD, NVL(PSV_STOR_CD, '0000000') 
           INTO lscard_div , lsbrand_cd , lsstor_cd
           FROM PARA_COMPANY CP
              , BRAND_MEMB   BM
          WHERE CP.COMP_CD  = BM.COMP_CD
    ...
    IF PSV_CRG_FG IN('1', '4', '9') THEN -- 결제구분[1:충전, 2:취소, 3:환불, 4:이전, 9:조정]
       IF (NVL(lsCustId, ' ') = ' ') AND (500000 < PSV_CRG_AMT + llsav_cash - lluse_cash) THEN -- 충전잔액이 50만원을 넘으면
          asRetVal := '1006';
    ...
    IF PSV_CRG_FG = '1' AND MOD(PSV_CRG_AMT, 10000) <> 0 THEN -- 결제구분[1:충전]
       asRetVal := '1009';
  ...
  BEGIN
    ...
    IF PSV_MEMB_DIV = '1' THEN -- 
       ...
       IF NVL(lsCustId, ' ') <> ' ' THEN -- 회원 ID 존재 시
          IF PSV_SALE_DIV IN('301', '303', '901') AND PSV_USE_AMT > 0 THEN -- 적립사용종류[301:사용, 302:사용반품, 303:사용누락, 901:조정]
             UPDATE C_CUST
    ...
    ELSE -- 
       ...
       (
           ...
           CASE WHEN PSV_SALE_DIV IN ('201', '301') THEN PSV_SAV_MLG ELSE PSV_SAV_MLG END,
           0           ,       'N'         ,
           ...
           CASE WHEN PSV_SALE_DIV IN ('201', '301') THEN PSV_USE_PT  ELSE PSV_USE_PT END,
           0           ,
           PSV_BRAND_CD,       PSV_STOR_CD ,
           PSV_POS_NO  ,       PSV_BILL_NO ,
           PSV_ORG_USE_DT,     CASE WHEN PSV_ORG_USE_SEQ = '0' THEN NULL ELSE TO_NUMBER(PSV_ORG_USE_SEQ) END,
       ...
       IF NVL(lsCustId, ' ') <> ' ' THEN -- 회원 ID 존재 시
          IF (PSV_SALE_DIV IN(       '201', '203', '901', '902', '903') AND PSV_SAV_MLG > 0) OR
             (PSV_SALE_DIV IN('101', '201', '203', '901', '902', '903') AND PSV_SAV_PT  > 0) THEN -- 적립사용종류[101:회원가입, 102:회원탈퇴 소멸, 201:적립, 202:적립반품, 203:적립누락, 301:사용, 302:사용반품, 303:사용누락, 901:조정, 902:이전, 903:기타]
             
             -- 최초 적립 DATA 생성
             IF PSV_SALE_DIV IN('201', '203', '901', '902', '903') AND PSV_SAV_MLG > 0 THEN
                 -- 최초구매 쿠폰 발행 존재 여부 
                 SELECT COUNT(*) INTO nRecCnt
  ...
  BEGIN
    ...
    FOR MYREC IN CUR_1 LOOP
        ...
        -- 잔액 체크
        IF (ABS(PSV_CRG_AMT) != (MYREC.CUST_SAV_CASH - MYREC.CUST_USE_CASH)) THEN
            asRetVal := '1005';
            asRetMsg := FC_GET_WORDPACK_MSG(PSV_COMP_CD, PSV_LANG_TP, '1010001426'); -- 환불금액이 잔액과 일치하지 않습니다.
       ...
       BEGIN
         ...
         ELSE
            IF lsUseStat IN ('10', '30', '31', '32', '33', '99') THEN -- 10:판매, 30:분실, 31:도난, 32:폐기, 33:유효기간 만료, 99:에러
               asRetVal := TO_NUMBER('10'||lsUseStat);
