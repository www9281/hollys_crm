CREATE OR REPLACE PACKAGE       PKG_SALE4032 AS

    ---------------------------------------------------------------------------------------------------
    --  Package Name     : PKG_SALE4032
    --  Description      : 시간대별 전일/전주대비 매출현황 
    -- Ref. Table        :
    ---------------------------------------------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY       PKG_SALE4032 AS
    ...
    */
        ...
        ls_sql_main := ''
            ...
            || CHR(13) || CHR(10) || Q'[                ) SC ,                                           ]'
            || CHR(13) || CHR(10) || Q'[                (                                                ]'
            || CHR(13) || CHR(10) || Q'[                SELECT /*+ INDEX (A IX01_PT_SALE_JTO) */         ]'
            || CHR(13) || CHR(10) || Q'[                    A.COMP_CD            AS COMP_CD,             ]'
            || CHR(13) || CHR(10) || Q'[                    A.SEC_DIV            AS SEC_DIV,             ]'
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, :PSV_GFR_DATE, DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT), 0))     AS BILL_CNT1,   ]'
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, :PSV_GFR_DATE, DECODE(:PSV_FILTER  , 'G', A.GRD_AMT, A.GRD_AMT - A.VAT_AMT), 0)) AS GRD_AMT1, ]'
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, TO_CHAR(TO_DATE(:PSV_GFR_DATE, 'YYYYMMDD') - 1  , 'YYYYMMDD'), DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT), 0))     AS BILL_CNT2, ]'
            ...
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, TO_CHAR(TO_DATE(:PSV_GFR_DATE, 'YYYYMMDD') - 7  , 'YYYYMMDD'), DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT), 0))     AS BILL_CNT3, ]'
            ...
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, TO_CHAR(TO_DATE(:PSV_GFR_DATE, 'YYYYMMDD') - 364, 'YYYYMMDD'), DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT), 0))     AS BILL_CNT4, ]'
            ...
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, :PSV_GFR_DATE, NVL2(B.STOR_CD, DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT),0), 0))     AS BILL_CNT12, ]'
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, :PSV_GFR_DATE, NVL2(C.STOR_CD, DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT),0), 0))     AS BILL_CNT13, ]'
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, :PSV_GFR_DATE, NVL2(D.STOR_CD, DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT),0), 0))     AS BILL_CNT14, ]'
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, TO_CHAR(TO_DATE(:PSV_GFR_DATE, 'YYYYMMDD') - 1   ,  'YYYYMMDD'), NVL2(B.STOR_CD, DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT),0), 0))     AS BILL_CNT21, ]'
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, TO_CHAR(TO_DATE(:PSV_GFR_DATE, 'YYYYMMDD') - 7   ,  'YYYYMMDD'), NVL2(C.STOR_CD, DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT),0), 0))     AS BILL_CNT31, ]'
            || CHR(13) || CHR(10) || Q'[                    SUM(DECODE(A.SALE_DT, TO_CHAR(TO_DATE(:PSV_GFR_DATE, 'YYYYMMDD') - 364 ,  'YYYYMMDD'), NVL2(D.STOR_CD, DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT),0), 0))     AS BILL_CNT41  ]'
            || CHR(13) || CHR(10) || Q'[                 FROM SALE_JTO A,                          ]'
