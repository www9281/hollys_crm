CREATE OR REPLACE PACKAGE       PKG_STCK1030 AS

    ---------------------------------------------------------------------------------------------------
    --  Package Name     : PKG_STCK1030
    --  Description      :  기간별 수불현황 
    -- Ref. Table        :
    ---------------------------------------------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY       PKG_STCK1030 AS
    ...
    ******************************************************************************/
    ...
    SELECT  A.L_CLASS_NM
         ...
         ,  CASE WHEN A.SURV_YN = 'N' THEN 0
            ...
            END                                                                                     AS DIFF_AMT
         ,  A.TOT_STOCK_QTY                                                                         AS TOT_STOCK_QTY    -- 최종재고수량
         ,  A.TOT_STOCK_AMT                                                                         AS TOT_STOCK_AMT    -- 최종재고금액
      ...
      FROM  (
                SELECT  S.COMP_CD
                     ...
                     ,  SUM(S.SALE_QTY + S.PROD_OUT_QTY + S.MV_OUT_QTY + S.RTN_QTY + S.FREE_1_QTY + S.FREE_2_QTY + S.FREE_3_QTY + S.FREE_4_QTY + S.FREE_5_QTY + S.FREE_6_QTY + S.FREE_7_QTY + S.FREE_8_QTY  + S.FREE_9_QTY + S.FREE_10_QTY + S.FREE_11_QTY + S.FREE_12_QTY + S.FREE_13_QTY + S.FREE_14_QTY)     AS TOT_OUT_QTY
                     ...
                     ,  SUM(  S.BEGIN_QTY + S.PRE_STOCK_QTY
                            + (S.ORD_QTY + S.MV_IN_QTY + S.ETC_IN_QTY)
                            - (S.SALE_QTY + S.PROD_OUT_QTY + S.MV_OUT_QTY + S.RTN_QTY + S.FREE_1_QTY + S.FREE_2_QTY + S.FREE_3_QTY + S.FREE_4_QTY + S.FREE_5_QTY + S.FREE_6_QTY + S.FREE_7_QTY + S.FREE_8_QTY  + S.FREE_9_QTY + S.FREE_10_QTY + S.FREE_11_QTY + S.FREE_12_QTY + S.FREE_13_QTY + S.FREE_14_QTY)
                            - (S.DSA_01_QTY + S.DSA_02_QTY + S.DSA_03_QTY + S.DSA_04_QTY + S.DSA_05_QTY + S.DSA_06_QTY + S.DSA_99_QTY)
                            + S.TOT_ADJ_QTY)                                            AS TOT_STOCK_QTY
                     ,  SUM(ROUND((  S.BEGIN_QTY + S.PRE_STOCK_QTY
                                   + (S.ORD_QTY + S.MV_IN_QTY + S.ETC_IN_QTY)
                                   - (S.SALE_QTY + S.PROD_OUT_QTY + S.MV_OUT_QTY + S.RTN_QTY + S.FREE_1_QTY + S.FREE_2_QTY + S.FREE_3_QTY + S.FREE_4_QTY + S.FREE_5_QTY + S.FREE_6_QTY + S.FREE_7_QTY + S.FREE_8_QTY  + S.FREE_9_QTY + S.FREE_10_QTY + S.FREE_11_QTY + S.FREE_12_QTY + S.FREE_13_QTY + S.FREE_14_QTY)
                                   - (S.DSA_01_QTY + S.DSA_02_QTY + S.DSA_03_QTY + S.DSA_04_QTY + S.DSA_05_QTY + S.DSA_06_QTY + S.DSA_99_QTY)
                                   + S.TOT_ADJ_QTY) * S.COST))                          AS TOT_STOCK_AMT
                              ...
                              FROM  (
                                 ...
                                 ,  (
                                        ...
                                        SELECT  A.COMP_CD
                                             ...
                                             ,  0           AS NOCHARGE_QTY
                                             ,  0           AS SURV_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '1'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_1_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '2'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_2_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '3'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_3_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '4'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_4_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '5'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_5_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '6'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_6_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '7'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_7_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '8'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_8_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '9'  AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_9_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '10' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_10_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '11' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_11_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '12' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_12_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '13' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_13_QTY
                                             ,  SUM ( CASE WHEN A.FREE_DIV = '14' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN SALE_QTY ELSE 0 END )    AS FREE_14_QTY
                                             ,  0           AS DSA_01_QTY
                                             ,  0           AS DSA_02_QTY
                                             ,  0           AS DSA_03_QTY
                                             ,  0           AS DSA_04_QTY
                                        ...
                                        SELECT  A.COMP_CD
                                             ...
                                             ,  0           AS FREE_13_QTY
                                             ,  0           AS FREE_14_QTY
                                             ,  SUM ( CASE WHEN A.ADJ_DIV = '01' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN DO_QTY ELSE 0 END )   AS DSA_01_QTY
                                             ,  SUM ( CASE WHEN A.ADJ_DIV = '02' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN DO_QTY ELSE 0 END )   AS DSA_02_QTY
                                             ,  SUM ( CASE WHEN A.ADJ_DIV = '03' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN DO_QTY ELSE 0 END )   AS DSA_03_QTY
                                             ,  SUM ( CASE WHEN A.ADJ_DIV = '04' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN DO_QTY ELSE 0 END )   AS DSA_04_QTY
                                             ...
                                             ,  SUM ( CASE WHEN A.ADJ_DIV = '99' AND A.SALE_DT BETWEEN PSV_GFR_DATE AND PSV_GTO_DATE THEN DO_QTY ELSE 0 END )   AS DSA_07_QTY
                                          FROM  SALE_CDR    A
                                         WHERE  A.COMP_CD   = PSV_COMP_CD
                                           AND  A.SALE_DT   BETWEEN LS_FR_DT AND LS_TO_DT
                                           AND  A.BRAND_CD  = PSV_BRAND_CD
