CREATE OR REPLACE PACKAGE       PKG_SALE1160 AS

   ---------------------------------------------------------------------------------------------------
   --  Package Name     : PKG_SALE1160
   --  Description      : 일정산내역(모스버거)
   -- Ref. Table        :
   ---------------------------------------------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY       PKG_SALE1160 AS
    ...
    ******************************************************************************/
    ...
    ls_sql_main :=      Q'[  SELECT  A.STOR_CD, A.STOR_NM, A.SALE_DT, A.SALE_DY, A.BILL_CNT, A.SALE_AMT, A.DC_AMT, A.GRD_AMT, A.VAT_AMT, A.NET_AMT, A.RTN_QTY, A.RTN_AMT, A.CUST_CNT, A.CUST_AMT ]'
    ...
    ||chr(13)||chr(10)||Q'[                  ,  SUM(A.BILL_CNT + A.RTN_BILL_CNT) AS BILL_CNT, DECODE(:PSV_CUST_DIV, 'C', SUM(A.ETC_M_CNT + A.ETC_F_CNT), SUM(A.BILL_CNT - A.RTN_BILL_CNT)) AS CUST_CNT, SUM(A.SALE_AMT) AS SALE_AMT, SUM(A.GRD_AMT) AS GRD_AMT, SUM(A.VAT_AMT) AS VAT_AMT, SUM(A.GRD_AMT - A.VAT_AMT) AS NET_AMT, SUM(A.DC_AMT + A.ENR_AMT) AS DC_AMT, SUM(A.RTN_QTY) AS RTN_QTY, SUM(A.RTN_AMT) AS RTN_AMT, DECODE(DECODE(:PSV_CUST_DIV, 'C', SUM(A.ETC_M_CNT + A.ETC_F_CNT), SUM(A.BILL_CNT - A.RTN_BILL_CNT)), 0, 0, SUM(A.GRD_AMT) / ( DECODE(:PSV_CUST_DIV, 'C', SUM(A.ETC_M_CNT + A.ETC_F_CNT), SUM(A.BILL_CNT - A.RTN_BILL_CNT)))) AS CUST_AMT ]'
