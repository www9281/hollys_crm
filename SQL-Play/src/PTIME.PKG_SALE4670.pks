CREATE OR REPLACE PACKAGE       PKG_SALE4670 AS
    PROCEDURE SP_TAB01 /*일자별 시간대별 매출 - 일자별*/
    ( 
    ...
    PROCEDURE SP_TAB02 /*일자별 시간대별 매출-시간대별*/
    ( 
        PSV_COMP_CD     IN  VARCHAR2 ,              -- 회사코드
        PSV_USER        IN  VARCHAR2 ,              -- LOGIN USER
        PSV_PGM_ID      IN  VARCHAR2 ,              -- Progrm ID
...
CREATE OR REPLACE PACKAGE BODY       PKG_SALE4670 AS
    ...
    ******************************************************************************/
    ...
    --            V_HD2 := V_HD2 || ' R04  CT' || TO_CHAR(i*5)   ;                -- 점유비
        ...
        ||chr(13)||chr(10)||Q'[      ,  SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD) AS TOT_CUST_CNT ]'
        ||chr(13)||chr(10)||Q'[      ,  DECODE(SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD), 0, 0,   ]'
        ||chr(13)||chr(10)||Q'[                    SUM(A.GRD_AMT) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD) / SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD))  AS TOT_CUST_AMT ]'
        ...
        ||chr(13)||chr(10)||Q'[                  ,  SUM(A.RTN_BILL_CNT) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT) AS RTN_BILL_CNT ]'
        ...
        ||chr(13)||chr(10)||Q'[                  ,  SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT) AS CUST_CNT ]'
        ||chr(13)||chr(10)||Q'[                  ,  DECODE(SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT), 0, 0, SUM(DECODE(:PSV_FILTER, 'G', A.GRD_AMT, 'T', A.SALE_AMT, A.GRD_AMT - A.VAT_AMT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT) / SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT)) AS CUST_AMT ]'
        ...
        ||chr(13)||chr(10)||Q'[                              ,  SUM(CASE WHEN A.SALE_DIV = '2' THEN 1 ELSE 0 END) AS RTN_BILL_CNT ]'
    ...
    ******************************************************************************/
    ...
    BEGIN
       ...
       ls_sql_crosstab_main :=  Q'[ SELECT  A.SALE_DT, MAX('(' || SUBSTR(A.SALE_DT,1,4) || '-' || SUBSTR(A.SALE_DT,5,2)  || '-' || SUBSTR(A.SALE_DT,7,2)|| ')(' || W.CODE_NM || ')') AS SALE_DT_NM ]'
        ...
        ||chr(13)||chr(10)||Q'[      ,  SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.SEC_DIV) AS TOT_CUST_CNT ]'
        ||chr(13)||chr(10)||Q'[      ,  DECODE(SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.SEC_DIV), 0, 0, SUM(A.GRD_AMT) OVER (PARTITION BY A.SEC_DIV) / SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.SEC_DIV)) AS TOT_CUST_AMT ]'
        ...
        ||chr(13)||chr(10)||Q'[                  ,  SUM(A.RTN_BILL_CNT) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT, A.SEC_DIV) AS RTN_BILL_CNT ]'
        ...
        ||chr(13)||chr(10)||Q'[                  ,  SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT, A.SEC_DIV) AS CUST_CNT ]'
        ||chr(13)||chr(10)||Q'[                  ,  DECODE(SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT, A.SEC_DIV), 0, 0, SUM(DECODE(:PSV_FILTER, 'G', A.GRD_AMT, 'T', A.SALE_AMT, A.GRD_AMT - A.VAT_AMT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT, A.SEC_DIV) / SUM(DECODE(:PSV_CUST_DIV, 'C', A.ETC_M_CNT + A.ETC_F_CNT, A.BILL_CNT - A.RTN_BILL_CNT)) OVER (PARTITION BY A.BRAND_CD, A.STOR_CD, A.SALE_DT, A.SEC_DIV)) AS CUST_AMT ]'
        ...
        ||chr(13)||chr(10)||Q'[                              ,  SUM(NVL(B.RTN_BILL_CNT, 0)) AS RTN_BILL_CNT ]'
