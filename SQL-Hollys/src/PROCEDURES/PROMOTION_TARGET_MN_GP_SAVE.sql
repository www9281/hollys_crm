--------------------------------------------------------
--  DDL for Procedure PROMOTION_TARGET_MN_GP_SAVE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."PROMOTION_TARGET_MN_GP_SAVE" (
-- ==========================================================================================
-- Author		:	권혁민
-- Create date	:	2017-11-16
-- Description	:	프로모션 대상메뉴군 등록/수정
-- Test			:	exec PROMOTION_TARGET_MN_GP_SAVE '002', '', '', '' 
-- ==========================================================================================
...
BEGIN  
        ...
        SELECT  P_PRMT_ID,
                ...
                TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 7)) AS USE_YN,
                TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 8)) AS INST_USER,
                (CASE WHEN TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 9)) = '@' THEN SYSDATE
                      ELSE TO_DATE(TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 9)),'YYYY-MM-DD') 
                 END
                )AS INST_DT,
                P_USER_ID,
                SYSDATE
...
END PROMOTION_TARGET_MN_GP_SAVE;
