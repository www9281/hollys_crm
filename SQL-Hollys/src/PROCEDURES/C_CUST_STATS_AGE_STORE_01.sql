--------------------------------------------------------
--  DDL for Procedure C_CUST_STATS_AGE_STORE_01
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_STATS_AGE_STORE_01" (
...
BEGIN
    ...
    -- Description   :   회원분석 연령대 현황 [가맹유형별 탭]
    -- ==========================================================================================
    OPEN O_CURSOR FOR
    SELECT
          ...
          -- 회원방문횟수          
          , (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND GET_AGE_GROUP(A.CUST_AGE) = CUST.CUST_AGE_GROUP AND B.STOR_TP = CUST.STOR_TP AND SALE_YM <= CUST.SALE_YM) AS MSS_VISIT_CNT  -- 방문횟수(누적)
          , CASE WHEN CUST.TOT_CUST_CNT = 0 THEN 0 ELSE (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND GET_AGE_GROUP(A.CUST_AGE) = CUST.CUST_AGE_GROUP AND B.STOR_TP = CUST.STOR_TP AND SALE_YM <= CUST.SALE_YM) / CUST.TOT_CUST_CNT END AS MSS_VISIT_CNT_AVG  -- 회원당 방문횟수
    ...
    FROM  (
            ...
            GROUP BY CUST.CUST_AGE_GROUP, STO.STOR_TP, L01.SALE_YM
          ) CUST
          ,(
            SELECT  
                  MSS.CUST_AGE_GROUP
    ...
    ORDER BY CUST.CUST_AGE_GROUP, CUST.STOR_TP, CUST.SALE_YM;
    
END C_CUST_STATS_AGE_STORE_01;
