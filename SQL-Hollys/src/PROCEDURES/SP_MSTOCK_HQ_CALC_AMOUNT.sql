--------------------------------------------------------
--  DDL for Procedure SP_MSTOCK_HQ_CALC_AMOUNT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MSTOCK_HQ_CALC_AMOUNT" 
(
...
--------------------------------------------------------------------------------
--  Procedure Name   : SP_MSTOCK_HQ_CALC_AMOUNT
--  Description      : 
--  Ref. Table       : MSTOCK_HQ[IU], DSTOCK[S]
--------------------------------------------------------------------------------
--  Create Date      : 2015-01-12 모스버거 TSMS PJT[총평균단가 적용]
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
        ...
        SELECT NVL(SUM(CASE WHEN BP.PARA_VAL = '1' THEN OD.ORD_CAMT + OD.ORD_CVAT ELSE OD.ORD_CAMT END), 0)
         ...
         WHERE OD.COMP_CD    = BP.COMP_CD(+)
           ...
           AND BP.PARA_CD(+) = '1007' -- 매입 부가세 설정[1:부가세포함, 2:부가세미포함]
    ...
    IF ls_cost_div = 'P' THEN -- 총평균법
       ll_in_amt := R.MV_IN_QTY * ll_in_cost / NVL(ll_ord_unit_qty, 1);
    END IF;

    -- 당월 출고단가, 출고금액 산정
    ll_out_qty  := R.MV_OUT_QTY + R.DISUSE_QTY + R.SALE_QTY + R.NOCHARGE_QTY;
    --ll_out_cost := CASE WHEN ll_out_qty = 0 THEN 0 ELSE ll_avg_cost END;
    ll_out_cost := ll_avg_cost;
    ll_out_amt  := ll_out_qty * ROUND(ll_out_cost / NVL(ll_ord_unit_qty, 1), 3);

    -- 당월 조정(비규명로스)단가, 조정(비규명로스)금액
    --ll_adj_cost := CASE WHEN R.ADJ_QTY = 0 THEN 0 ELSE ll_avg_cost END;
    ll_adj_cost := ll_avg_cost;
