--------------------------------------------------------
--  DDL for Procedure SP_MEPA1050L2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MEPA1050L2" /* 회원 크라운 적립현황(회원등급) */
(
...
IS
/******************************************************************************
   NAME:       SP_MEPA1050L2  회원 크라운 적립현황(회원등급) 
   PURPOSE:
   ...
   1.1        2015-06-19         1. 연령대와 회원등급의 방문회원수를 일별 통계와 동일하게 하기 위해 GROUP BY 절에 USE_DT 추가
   NOTES:

      OBJECT NAME:     SP_MEPA1050L2
      SYSDATE:         2015-06-18
      USERNAME:
      TABLE NAME:
******************************************************************************/
   ...
   ls_sql_crosstab_main :=
    ...
    LOOP
        BEGIN
            ...
            V_HD1 := V_HD1 || ''''   || qry_hd(i).LVL_NM || ''' CT' || TO_CHAR(i*5 - 4) || ',' ;
            V_HD1 := V_HD1 || ''''   || qry_hd(i).LVL_NM || ''' CT' || TO_CHAR(i*5 - 3) || ',' ;
            V_HD1 := V_HD1 || ''''   || qry_hd(i).LVL_NM || ''' CT' || TO_CHAR(i*5 - 2) || ',' ;
            V_HD1 := V_HD1 || ''''   || qry_hd(i).LVL_NM || ''' CT' || TO_CHAR(i*5 - 1) || ',' ;
            V_HD1 := V_HD1 || ''''   || qry_hd(i).LVL_NM || ''' CT' || TO_CHAR(i*5)  ;
