--------------------------------------------------------
--  DDL for Procedure SP_MEAN1010L0_2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MEAN1010L0_2" /* 회원통계정보-전체회원현황(전체) */
(
...
*/
    ...
    ls_sql_main :=                  '   SELECT  /*+ NO_MERGE LEADING(CST) */ '
                ...
                ||chr(13)||chr(10)||'                ELSE ROUND((JDS.TOT_GRD_AMT - MSS.CST_GRD_AMT) / (JDS.TOT_BILL_CNT - MSS.CST_BILL_CNT))                     '
                ...
                ||chr(13)||chr(10)||'                ELSE ROUND((SUM(JDS.TOT_GRD_AMT - MSS.CST_GRD_AMT)OVER()) / (SUM(JDS.TOT_BILL_CNT - MSS.CST_BILL_CNT) OVER()))  '
                ...
                ||chr(13)||chr(10)||'                 , SUM(CASE WHEN CST.JOIN_DT <=   V01.STD_YM||''31'' AND SUBSTR(NVL(CST.LEAVE_DT, ''99991231''), 1, 8) >= V01.STD_YM||''31'' THEN 1 ELSE 0 END) TOT_CUST_CNT '
                ||chr(13)||chr(10)||'                 , SUM(CASE WHEN CST.JOIN_DT LIKE V01.STD_YM||''%''  AND SUBSTR(NVL(CST.LEAVE_DT, ''99991231''), 1, 8) >= V01.STD_YM||''31'' THEN 1 ELSE 0 END) NEW_CUST_CNT '
                ...
                ||chr(13)||chr(10)||'                           WHERE   MVL.COMP_CD = CST.COMP_CD  '
                ||chr(13)||chr(10)||'                           AND     MVL.CUST_ID = CST.CUST_ID  '
                ||chr(13)||chr(10)||'                           AND     MVL.SALE_YM = V01.STD_YM   '
                ||chr(13)||chr(10)||'                           AND    ('''||PSV_CUST_GRADE||''' IS NULL OR MVL.CUST_LVL = '''||PSV_CUST_GRADE||''')'
                ||chr(13)||chr(10)||'                     )                                        '
