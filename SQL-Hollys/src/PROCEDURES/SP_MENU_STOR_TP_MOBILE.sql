--------------------------------------------------------
--  DDL for Procedure SP_MENU_STOR_TP_MOBILE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MENU_STOR_TP_MOBILE" 
/******************************************************************************
   NAME     :  SP_MENU_STOR_TP_MOBILE
   PURPOSE  :  메뉴그룹별 메뉴 저장, 삭제 수정

   NOTES:

   Automatically available Auto Replace Keywords:
      Object Name:     SP_MENU_STOR_TP_MOBILE
      Sysdate:         2009-12-15
...
BEGIN
     ...
     IF( p_proc_fg = 'S' ) THEN
          BEGIN
               ...
               FROM      W_MENU_STOR_TP_MOBILE
               WHERE     COMP_CD       = p_comp_cd
               AND       MENU_CD       = p_menu_cd
               AND       STOR_TP       = p_menu_grp;

               IF( s_menu_cd_insert_yn = 'N') THEN RETURN;
               END IF;

               INSERT INTO W_MENU_STOR_TP_MOBILE (COMP_CD, STOR_TP , MENU_CD ,AUTHORITY, USE_YN , INST_DT , INST_USER , UPD_DT ,UPD_USER)
               ...
               BEGIN
                    ...
                    AND       MENU_CD = (
                                   SELECT    MENU_REF
                                   FROM      W_MENU_MOBILE       A1
                                   ,         W_MENU_STOR_TP_MOBILE A2
                                   WHERE     A2.COMP_CD       = p_comp_cd
                                   AND       A2.MENU_CD       = p_menu_cd
                                   AND       A2.STOR_TP       = p_menu_grp
               ...
               if( s_menu_user_ref = 0  AND s_menu_ref <> 0 ) THEN
                    SP_MENU_STOR_TP_MOBILE(p_comp_cd, p_menu_grp ,  s_menu_ref , 'S' , p_user_id , liv_msg_code , liv_msg_code);
               END IF;
          END;
     ...
     ELSE IF( p_proc_fg = 'D') THEN
          ...
          BEGIN
              DELETE  FROM W_MENU_STOR_TP_MOBILE
              WHERE   COMP_CD       = p_comp_cd
              AND     MENU_CD       = p_menu_cd
              AND     STOR_TP       = p_menu_grp;

              DELETE  FROM W_MENU_STORE_USER_MOBILE
              WHERE   COMP_CD  = p_comp_cd
              AND     MENU_CD  = p_menu_cd
              AND     STOR_CD IN (
                        SELECT  STOR_CD
