--------------------------------------------------------
--  DDL for Procedure C_CUST_STATS_ALL_STORE_04
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_STATS_ALL_STORE_04" (
...
BEGIN
    ...
    -- Description   :   회원분석 전체회원 현황 [SC별 탭]
    -- ==========================================================================================
    OPEN O_CURSOR FOR
    SELECT 
          ...
          -- 회원방문횟수          
          , (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND B.TEAM_CD = CUST.TEAM_CD AND B.SV_USER_ID = CUST.SV_USER_ID AND SALE_YM <= CUST.SALE_YM) AS MSS_VISIT_CNT  -- 방문횟수(누적)
          , CASE WHEN CUST.TOT_CUST_CNT = 0 THEN 0 ELSE (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND B.TEAM_CD = CUST.TEAM_CD AND B.SV_USER_ID = CUST.SV_USER_ID AND SALE_YM <= CUST.SALE_YM) / CUST.TOT_CUST_CNT END AS MSS_VISIT_CNT_AVG  -- 회원당 방문횟수
    ...
    FROM  (
            ...
            GROUP BY STO.TEAM_CD, STO.SV_USER_ID, L01.SALE_YM
          ) CUST
          ,(
            SELECT  
                  MSS.TEAM_CD
    ...
    WHERE  CUST.TEAM_CD        = JDS.TEAM_CD
    AND    CUST.SV_USER_ID     = JDS.SV_USER_ID
    AND    CUST.SALE_YM        = JDS.SALE_YM
    AND    JDS.TEAM_CD         = MSS.TEAM_CD (+)
    AND    JDS.SV_USER_ID      = MSS.SV_USER_ID (+)
    AND    JDS.SALE_YM         = MSS.SALE_YM (+)
    ORDER BY CUST.TEAM_CD, CUST.SV_USER_ID, JDS.SALE_YM;
    
END C_CUST_STATS_ALL_STORE_04;
