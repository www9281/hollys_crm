--------------------------------------------------------
--  DDL for Procedure C_CUST_STATS_ALL_STORE_02
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_STATS_ALL_STORE_02" (
...
BEGIN
    ...
    -- Description   :   회원분석 전체회원 현황 [상권별 탭]
    -- ==========================================================================================
    OPEN O_CURSOR FOR
    SELECT  
          ...
          -- 회원방문횟수          
          , (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND B.TRAD_AREA = CUST.TRAD_AREA AND SALE_YM <= CUST.SALE_YM) AS MSS_VISIT_CNT  -- 방문횟수(누적)
          , CASE WHEN CUST.TOT_CUST_CNT = 0 THEN 0 ELSE (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND B.TRAD_AREA = CUST.TRAD_AREA AND SALE_YM <= CUST.SALE_YM) / CUST.TOT_CUST_CNT END AS MSS_VISIT_CNT_AVG  -- 회원당 방문횟수
    ...
    FROM  (
            ...
            GROUP BY STO.TRAD_AREA, L01.SALE_YM
          ) CUST
          ,(
            SELECT  
                  MSS.TRAD_AREA
    ...
    WHERE  CUST.TRAD_AREA      = JDS.TRAD_AREA
    AND    CUST.SALE_YM        = JDS.SALE_YM
    AND    JDS.TRAD_AREA       = MSS.TRAD_AREA (+)
    AND    JDS.SALE_YM         = MSS.SALE_YM (+)
    ORDER BY CUST.TRAD_AREA, JDS.SALE_YM;
    
END C_CUST_STATS_ALL_STORE_02;
