--------------------------------------------------------
--  DDL for Procedure BATCH_TRANS_SALE_DATA_FRPOS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."BATCH_TRANS_SALE_DATA_FRPOS" 
IS
  V_RETC  VARCHAR2(1000);
BEGIN 
  -- ==========================================================================================
  -- Author        :   이춘승
  ...
  IF V_RETC IS NULL THEN 
    BEGIN
      ...
      IF SQL%ROWCOUNT > 0 THEN 
        UPDATE SALE_HD@HPOSDB
        SET    SEND_YN = 'Y'
        WHERE  COMP_CD = '016'
        AND    (SALE_DT,BRAND_CD,STOR_CD,POS_NO,BILL_NO)
            IN (SELECT SALE_DT
                     , BRAND_CD
                     , STOR_CD
                     , POS_NO
  ...
  IF V_RETC IS NULL THEN 
    BEGIN
      ...
      IF SQL%ROWCOUNT > 0 THEN 
        UPDATE SALE_DT@HPOSDB
        SET    SEND_YN = 'Y'
        WHERE  COMP_CD = '016'
        AND    (SALE_DT,BRAND_CD,STOR_CD,POS_NO,BILL_NO,SEQ)
            IN (SELECT SALE_DT
                     , BRAND_CD
                     , STOR_CD
                     , POS_NO
  ...
  IF V_RETC IS NULL THEN 
    BEGIN
      ...
      IF SQL%ROWCOUNT > 0 THEN 
        UPDATE SALE_DC@HPOSDB
        SET    SEND_YN = 'Y'
        WHERE  COMP_CD = '016'
        AND    (SALE_DT,BRAND_CD,STOR_CD,POS_NO,BILL_NO,SEQ,DC_SEQ)
            IN (SELECT SALE_DT
                     , BRAND_CD
                     , STOR_CD
                     , POS_NO
    ...
    EXCEPTION
      WHEN OTHERS THEN 
        ROLLBACK;
        V_RETC := 'SALE_DC-(' || SQLERRM || ')';
    END;
  END IF;  

END BATCH_TRANS_SALE_DATA_FRPOS;
