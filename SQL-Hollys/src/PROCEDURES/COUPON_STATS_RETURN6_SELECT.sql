--------------------------------------------------------
--  DDL for Procedure COUPON_STATS_RETURN6_SELECT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."COUPON_STATS_RETURN6_SELECT" 
(
...
(
    SELECT 
         ...
         , ROUND(COUNT(*) / NULLIF(COUNT(DISTINCT A.CUST_ID),1) ,0)       AS CNT
    ...
    MAX(CNT) FOR(LVL_CD) IN ('103'  RR ,'102' GG ,'101' SS,'TOT'  TT)
)A
ORDER BY DIV 
    ;
...
END COUPON_STATS_RETURN6_SELECT;
