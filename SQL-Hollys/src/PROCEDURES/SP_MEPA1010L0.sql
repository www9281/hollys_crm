--------------------------------------------------------
--  DDL for Procedure SP_MEPA1010L0
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MEPA1010L0" /* 회원 포인트 적립현황(일별) */
(
...
/******************************************************************************
   NAME:       SP_MEPA1010L0  회원 포인트 적립현황(일별) 
   PURPOSE:
...
/* 가로축 데이타 FETCH */
   ls_sql_crosstab_main :=
    ...
    ls_sql_main :=                   '   SELECT  V03.BRAND_CD                                        '
                ...
                ||chr(13)||chr(10)|| '         , SUM(V03.DAY_SAV_PT  ) OVER(PARTITION BY V03.STOR_CD) AS TOT_SAV_PT          '
