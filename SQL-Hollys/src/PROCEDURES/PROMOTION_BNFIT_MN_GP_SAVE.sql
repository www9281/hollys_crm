--------------------------------------------------------
--  DDL for Procedure PROMOTION_BNFIT_MN_GP_SAVE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."PROMOTION_BNFIT_MN_GP_SAVE" (
-- ==========================================================================================
-- Author		:	권혁민
-- Create date	:	2017-11-16
-- Description	:	프로모션 혜택메뉴군 등록/수정
-- Test			:	exec PROMOTION_BNFIT_MN_GP_SAVE '002', '', '', '' 
-- ==========================================================================================
...
BEGIN  
       ...
       SELECT   P_PRMT_ID,
                ...
                TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 5)) AS USE_YN,
                ...
                (CASE WHEN TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 8)) = '@' THEN NULL
                      ELSE TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 8))
                 END
                ) AS SALE_RATE,
                TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 9)) AS BNFIT_DIV,
                (CASE WHEN TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 10)) = '@' THEN NULL
                      ELSE TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 10))
                 END
                ) AS GIVE_REWARD,                
                TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 11)) AS INST_USER,
                (CASE WHEN TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 12)) = '@' THEN SYSDATE
                      ELSE TO_DATE(TRIM(REGEXP_SUBSTR(D, '[^' || L_COLUMN || ']+', 1, 12)),'YYYY-MM-DD') 
                 END
                )AS INST_DT,
                P_USER_ID,
                SYSDATE
...
END PROMOTION_BNFIT_MN_GP_SAVE;
