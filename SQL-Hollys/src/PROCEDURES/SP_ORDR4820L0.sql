--------------------------------------------------------
--  DDL for Procedure SP_ORDR4820L0
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_ORDR4820L0" /* 주문확정현황_일자별 */
(
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            V_HD := V_HD || '''' || SUBSTR(qry_hd(i).SHIP_DT, 1, 4) || '-' ||  SUBSTR(qry_hd(i).SHIP_DT, 5, 2) || '-' || SUBSTR(qry_hd(i).SHIP_DT, 7, 2) || '''' ;
        END;
    END LOOP;
