--------------------------------------------------------
--  DDL for Procedure BATCH_STAT_MEMBER_TOT3
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."BATCH_STAT_MEMBER_TOT3" (
...
BEGIN
  ...
  -- Description   :   전체 회원 현황 자료 생성...상권별
  --                    월별, 분기별, 년도별
  -- ==========================================================================================
  ...
  IF PI_YYMM    IS NULL OR
    ...
    PO_RETC := 'NG';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT3', '전체 회원 현황 자료 생성', '3.상권별:::파라메터 오류', PO_RETC, V_RETC);
    RETURN;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
    BEGIN
      ...
      USING (SELECT 'M'                            INFO_DIV1
             ...
             FROM   (
                     ...
                     GROUP BY A.COMP_CD,A.BRAND_CD,C.SSS_DIV,B.TRAD_AREA
                    )       T0
                  , (--전체회원,신규회원
                     SELECT COMP_CD
                          , BRAND_CD
                      ...
                      GROUP BY COMP_CD,BRAND_CD,SSS_DIV,TRAD_AREA
                    )       T1
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,A.TRAD_AREA
                    )       T2
                  ...
                  , (
                     ...
                     FROM   TEMP_STORE
                     WHERE  COMP_CD = V_COMP_CD
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV_M,TRAD_AREA
                    )       T4
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT3', '전체 회원 현황 자료 생성', '3-1-1.상권별-월별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --1-2.월별_합
  ...
  IF PO_RETC IS NULL THEN
    BEGIN
      ...
      USING (SELECT 'M'                            INFO_DIV1
             ...
             FROM   (
                     ...
                     AND    A.COMP_CD  = B.COMP_CD
                     AND    A.BRAND_CD = B.BRAND_CD
                     GROUP BY A.COMP_CD,A.BRAND_CD,B.TRAD_AREA
                    )       T0
                  , (--전체회원,신규회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     AND    STOR_TG    = '-'
                     AND    STOR_CD    = '-'
                     GROUP BY COMP_CD,BRAND_CD,TRAD_AREA
                    )       T1
                  , (--구매회원
                     SELECT A.COMP_CD                                                       COMP_CD
                          , A.BRAND_CD                                                      BRAND_CD
                     ...
                     FROM   (
                          ...
                          , TEMP_CUST_STAT1 B
                     WHERE  A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.TRAD_AREA
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원/비회원
                     SELECT COMP_CD
                          , BRAND_CD
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT3', '전체 회원 현황 자료 생성', '3-1-2.상권별-월별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF (PO_RETC IS NULL) AND (PI_QTFV = TRUE) THEN
      ...
      USING (SELECT 'Q'                            INFO_DIV1
             ...
             FROM   (
                  ...
                  , (
                     ...
                     FROM   TEMP_STORE
                     WHERE  COMP_CD = V_COMP_CD
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV_Q,TRAD_AREA
                    )       T4
             WHERE  T0.COMP_CD   = T1.COMP_CD(+)
             AND    T0.BRAND_CD  = T1.BRAND_CD(+)
             AND    T0.SSS_DIV   = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT3', '전체 회원 현황 자료 생성', '3-2-1.상권별-분기별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --2-2.분기별_합
  ...
  IF (PO_RETC IS NULL) AND (PI_QTFV = TRUE) THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT3', '전체 회원 현황 자료 생성', '3-2-2.상권별-분기별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF (PO_RETC IS NULL) AND (PI_YTFV = TRUE) THEN
      ...
      USING (SELECT 'Y'                            INFO_DIV1
             ...
             FROM   (
                  ...
                  , (
                     ...
                     FROM   TEMP_STORE
                     WHERE  COMP_CD = V_COMP_CD
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV_Y,TRAD_AREA
                    )       T4
             WHERE  T0.COMP_CD   = T1.COMP_CD(+)
             AND    T0.BRAND_CD  = T1.BRAND_CD(+)
             AND    T0.SSS_DIV   = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT3', '전체 회원 현황 자료 생성', '3-3-1.상권별-년도별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --3-2.년도별_합
  ...
  IF (PO_RETC IS NULL) AND (PI_YTFV = TRUE) THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT3', '전체 회원 현황 자료 생성', '3-3-2.상권별-년도별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
    COMMIT;
    PO_RETC := 'OK';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT3', '전체 회원 현황 자료 생성', '3.상권별:::'||V_YYMM, PO_RETC, V_RETC);
  END IF;

END BATCH_STAT_MEMBER_TOT3;
