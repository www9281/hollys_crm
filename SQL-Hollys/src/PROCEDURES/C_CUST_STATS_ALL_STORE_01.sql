--------------------------------------------------------
--  DDL for Procedure C_CUST_STATS_ALL_STORE_01
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_STATS_ALL_STORE_01" (
...
BEGIN
    ...
    -- Description   :   회원분석 전체회원 현황 [가맹유형별 탭]
    -- ==========================================================================================
    OPEN O_CURSOR FOR
    SELECT 
          GET_COMMON_CODE_NM('00565', JDS.STOR_TP, 'KOR') AS DIV_TYPE1
          ...
          -- 회원방문횟수          
          , (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND B.STOR_TP = CUST.STOR_TP AND SALE_YM <= CUST.SALE_YM) AS MSS_VISIT_CNT  -- 방문횟수(누적)
          , CASE WHEN CUST.TOT_CUST_CNT = 0 THEN 0 ELSE (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND B.STOR_TP = CUST.STOR_TP AND SALE_YM <= CUST.SALE_YM) / CUST.TOT_CUST_CNT END AS MSS_VISIT_CNT_AVG  -- 회원당 방문횟수
    ...
    FROM  (
            ...
            GROUP BY STO.STOR_TP, L01.SALE_YM
          ) CUST
          ,(
            SELECT  
                  MSS.STOR_TP
    ...
    WHERE  CUST.STOR_TP  = JDS.STOR_TP
    AND    CUST.SALE_YM  = JDS.SALE_YM
    AND    JDS.STOR_TP   = MSS.STOR_TP (+)
    AND    JDS.SALE_YM   = MSS.SALE_YM (+)
    ORDER BY CUST.STOR_TP, JDS.SALE_YM;
    
END C_CUST_STATS_ALL_STORE_01;
