--------------------------------------------------------
--  DDL for Procedure SP_MENU_STOR
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MENU_STOR" 
/******************************************************************************
   NAME:       SP_MENU_STOR
   PURPOSE:
   ...
   ---------  ----------  ---------------  ------------------------------------
   1.0        2009-12-15          1. Created this procedure.
...
,    p_pgm_menu_grp IN   VARCHAR2
,    p_menu_cd      IN   NUMBER
,    p_proc_fg      IN   CHAR
,    p_upd_user     IN   VARCHAR2
...
BEGIN
     ...
     IF( p_proc_fg = 'S' ) THEN
          BEGIN
               ...
               AND       PGM_MENU_GRP = p_pgm_menu_grp;

               IF( s_menu_cd_insert_yn = 'N') THEN RETURN;
               END IF;

               INSERT INTO W_MENU_GROUP (COMP_CD, PGM_MENU_GRP, MENU_CD, AUTHORITY, INST_DT, INST_USER, UPD_DT, UPD_USER)
               VALUES(p_comp_cd, p_pgm_menu_grp, p_menu_cd, '', sysdate, p_upd_user, sysdate, p_upd_user);
               ...
               BEGIN
                    ...
                    AND       MENU_CD = (
                                   ...
                                   AND       A2.PGM_MENU_GRP = p_pgm_menu_grp
               ...
               if( s_menu_user_ref = 0  AND s_menu_ref <> 0 ) THEN
                    SP_MENU_STOR(p_comp_cd, p_pgm_menu_grp,  s_menu_ref, 'S', p_upd_user, liv_msg_code, liv_msg_code );
               END IF;
          END;
