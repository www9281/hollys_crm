--------------------------------------------------------
--  DDL for Procedure BATCH_STAT_MEMBER_TOT5
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."BATCH_STAT_MEMBER_TOT5" (
...
BEGIN
  ...
  -- Description   :   전체 회원 현황 자료 생성...SC별
  --                    월별, 분기별, 년도별
  -- ==========================================================================================
  ...
  IF PI_YYMM    IS NULL OR
    ...
    PO_RETC := 'NG';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT5', '전체 회원 현황 자료 생성', '5.SC별:::파라메터 오류', PO_RETC, V_RETC);
    RETURN;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
    BEGIN
      ...
      USING (SELECT 'M'                            INFO_DIV1
             ...
             FROM   (
                     ...
                     GROUP BY A.COMP_CD,A.BRAND_CD,C.SSS_DIV,B.SV_USER_ID
                    )       T0
                  , (--전체회원,신규회원
                     SELECT COMP_CD
                          , BRAND_CD
                      ...
                      GROUP BY COMP_CD,BRAND_CD,SSS_DIV,SC_CD
                    )       T1
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     FROM   (
                             ...
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_M,BB.SV_USER_ID
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,A.SC_CD
                    )       T2
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT5', '전체 회원 현황 자료 생성', '5-1-1.SC별-월별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --1-2.월별_합
  ...
  IF PO_RETC IS NULL THEN
    BEGIN
      ...
      USING (SELECT 'M'                            INFO_DIV1
             ...
             FROM   (
                     ...
                     AND    A.COMP_CD  = B.COMP_CD
                     AND    A.BRAND_CD = B.BRAND_CD
                     GROUP BY A.COMP_CD,A.BRAND_CD,B.SV_USER_ID
                    )       T0
                  , (--전체회원,신규회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     AND    STOR_TG    = '-'
                     AND    STOR_CD    = '-'
                     GROUP BY COMP_CD,BRAND_CD,SC_CD
                    )       T1
                  , (--구매회원
                     SELECT A.COMP_CD                                                       COMP_CD
                          , A.BRAND_CD                                                      BRAND_CD
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SV_USER_ID
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SC_CD
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원/비회원
                     SELECT COMP_CD
                          , BRAND_CD
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT5', '전체 회원 현황 자료 생성', '5-1-2.SC별-월별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF (PO_RETC IS NULL) AND (PI_QTFV = TRUE) THEN
             ...
             FROM   (
                  ...
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_Q,BB.SV_USER_ID
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,A.SC_CD
                  ...
                  , (
                     ...
                     FROM   TEMP_STORE
                     WHERE  COMP_CD = V_COMP_CD
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV_Q,SV_USER_ID
                    )       T4
             WHERE  T0.COMP_CD  = T1.COMP_CD(+)
             AND    T0.BRAND_CD = T1.BRAND_CD(+)
             AND    T0.SSS_DIV  = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT5', '전체 회원 현황 자료 생성', '5-2-1.SC별-분기별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --2-2.분기별_합
  ...
  IF (PO_RETC IS NULL) AND (PI_QTFV = TRUE) THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT5', '전체 회원 현황 자료 생성', '5-2-2.SC별-분기별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF (PO_RETC IS NULL) AND (PI_YTFV = TRUE) THEN
             ...
             FROM   (
                  ...
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_Y,BB.SV_USER_ID
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,A.SC_CD
                  ...
                  , (
                     ...
                     FROM   TEMP_STORE
                     WHERE  COMP_CD = V_COMP_CD
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV_Y,SV_USER_ID
                    )       T4
             WHERE  T0.COMP_CD  = T1.COMP_CD(+)
             AND    T0.BRAND_CD = T1.BRAND_CD(+)
             AND    T0.SSS_DIV  = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT5', '전체 회원 현황 자료 생성', '5-3-1.SC별-년도별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --3-2.년도별_합
  ...
  IF (PO_RETC IS NULL) AND (PI_YTFV = TRUE) THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT5', '전체 회원 현황 자료 생성', '5-3-2.SC별-년도별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
    COMMIT;
    PO_RETC := 'OK';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT5', '전체 회원 현황 자료 생성', '5.SC별:::'||V_YYMM, PO_RETC, V_RETC);
  END IF;

END BATCH_STAT_MEMBER_TOT5;
