--------------------------------------------------------
--  DDL for Procedure SP_SYNC_TSMS_PCRM
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_SYNC_TSMS_PCRM" 
(
    ...
    ***********************************************************************/
    ...
    FOR MYREC1 IN CUR_1 LOOP
        ...
        IF MYREC1.SYNC_DIV = 'A' THEN
            vSQLTXT1 := 'INSERT INTO '||MYREC1.TBL_ID||' ('||vSQLTXT2||')'||
                        '   SELECT '||vSQLTXT2||
            ...
            /**** 미사용 (통계자료 동기화) ****
            -- 최초 매출일자 취득
            vSQLTXT1 := '   SELECT  MIN(SALE_DT) FROM PAUL.'||MYREC1.TBL_ID||
                        '   WHERE   SALE_DT < TO_CHAR(SYSDATE, ''YYYYMMDD'')'||
                        '   AND    '||MYREC1.COMP_COL1||' >= TO_DATE( '||MYREC1.COMP_VAL1||',''YYYYMMDDHH24MISS'')'||
                        '   AND    '||MYREC1.COMP_COL1||' <  TO_DATE( '||vSYSDATETIME    ||',''YYYYMMDDHH24MISS'')';
            --DBMS_OUTPUT.PUT_LINE('2-2'||vSQLTXT1);
            EXECUTE IMMEDIATE vSQLTXT1 INTO vMIN_SALE_DT;
            
            EXIT WHEN vMIN_SALE_DT IS NULL;
            
            -- 최초 매출일자로 UPDATE
            vSQLTXT1 := '   UPDATE  SYNC_TBL_LST '||
                        '   SET     COMP_VAL1 = '''||vMIN_SALE_DT||''''||
                        '   WHERE   MAIN_TBL  = '''||MYREC1.TBL_ID||'''';
            
            --DBMS_OUTPUT.PUT_LINE('2-3'||vSQLTXT1);
            EXECUTE IMMEDIATE vSQLTXT1;
            ***********************************/
            ...
            --DBMS_OUTPUT.PUT_LINE('2-4'||vSQLTXT1);
