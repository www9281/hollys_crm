--------------------------------------------------------
--  DDL for Procedure STORE_CROWN_SAV_AGE_SELECT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."STORE_CROWN_SAV_AGE_SELECT" (
...
--------------------------------- 매장별 크라운 적립현황(연령대) ----------------------------------
    ...
    v_query := 
              ...
              SELECT  V03.BRAND_CD
                    ...
                    , CASE  WHEN SUM(V03.DAY_CUST_CNT) OVER(PARTITION BY V03.STOR_CD) = 0 
                            THEN 0.0
                            ELSE NVL(ROUND(SUM(V03.DAY_SAV_MLG) OVER(PARTITION BY V03.STOR_CD) / SUM(V03.DAY_CUST_CNT) OVER(PARTITION BY V03.STOR_CD), 2), 0.0)
...
END STORE_CROWN_SAV_AGE_SELECT;
