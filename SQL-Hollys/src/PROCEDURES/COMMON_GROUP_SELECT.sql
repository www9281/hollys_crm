--------------------------------------------------------
--  DDL for Procedure COMMON_GROUP_SELECT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."COMMON_GROUP_SELECT" (
        P_CODE_TP  IN    VARCHAR2,
        N_CODE_NM  IN    VARCHAR2,
        N_USE_YN   IN    CHAR,
...
BEGIN
        ...
        WHERE CODE_NM LIKE '%' || N_CODE_NM || '%'
        ORDER BY 
                CODE_TP ASC,
                CODE_CD ASC;
END COMMON_GROUP_SELECT;
