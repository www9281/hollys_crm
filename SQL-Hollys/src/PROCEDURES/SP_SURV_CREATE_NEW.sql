--------------------------------------------------------
--  DDL for Procedure SP_SURV_CREATE_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_SURV_CREATE_NEW" 
...
---------------------------------------------------------------------------------------------------
--  Procedure Name   : SP_SURV_CREATE_NEW               [STCK4222M0.jsp]
         ...
         Exception When DUP_VAL_ON_INDEX Then
            ...
            UPDATE  SURV_STOCK_DT
               SET  ORD_UNIT_QTY   = r.ord_unit_qty
                 ...
                 ,  ADJ_QTY        = (CASE WHEN ORD_SURV_QTY = 0 AND SURV_QTY = 0 THEN -1 * r.inventory_qty ELSE (ORD_UNIT_QTY*ORD_SURV_QTY + SALE_UNIT_QTY*SALE_SURV_QTY + SURV_QTY) - (r.inventory_qty) END)
                 ,  S_CONFIRM_YN   = DECODE(r.inventory_qty, 0, 'N', 'Y')
                 ,  UPD_USER       = asUserId
                 ,  UPD_DT         = SYSDATE
             WHERE COMP_CD   = asCompCd
               AND SURV_DT   = asSurvDt
