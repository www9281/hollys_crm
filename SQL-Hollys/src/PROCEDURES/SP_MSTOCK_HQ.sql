--------------------------------------------------------
--  DDL for Procedure SP_MSTOCK_HQ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MSTOCK_HQ" 
(
  psv_comp_cd   IN  STRING, -- 회사코드
  psv_brand_cd  IN  STRING, -- 영업조직
  psv_ym        IN  STRING  -- 작업년월
) IS
--------------------------------------------------------------------------------
--  Procedure Name   : SP_MSTOCK_HQ
--  Description      : 월수불[본사] 테이블에 기초재고를 이월함(SP_MSTOCK_JOB에서 호출)
--  Ref. Table       : MSTOCK_HQ[IU], DSTOCK[S]
--------------------------------------------------------------------------------
--  Create Date      : 2015-01-15 모스버거 TSMS PJT
--  Modify Date      : 2015-01-15
--------------------------------------------------------------------------------
BEGIN
  MERGE INTO MSTOCK_HQ A
  USING (
            SELECT  COMP_CD
                 ,  BRAND_CD
                 ,  ITEM_CD
