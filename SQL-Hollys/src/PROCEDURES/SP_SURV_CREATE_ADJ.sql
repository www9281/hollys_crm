--------------------------------------------------------
--  DDL for Procedure SP_SURV_CREATE_ADJ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_SURV_CREATE_ADJ" 
...
---------------------------------------------------------------------------------------------------
--  Procedure Name   : SP_SURV_CREATE_ADJ               [STCK4224M0.jsp]
--  Description      : 재고실사 조정 데이터 생성
...
CURSOR C_Stock IS
   WITH ST AS
             ( SELECT   COMP_CD 
                 ...
                 FROM TABLE(CAST ( FC_GET_STOCK_FQTY_CD_DAY (asCompCd, 'KOR', asBrandCd, asStorCd,asSurvDt) AS TBL_STOCK_FQTY_CD_DAY ) )
             )
   ...
   */
   ...
   FOR r IN C_Stock LOOP
              ...
              INSERT INTO SURV_STOCK_DT
                ...
                VALUES
                      (  asCompCd
                       ...
                       , r.sale_unit_qty
                       , r.inventory_qty
                       , (CASE WHEN r.inventory_qty < 0 THEN 0 ELSE TRUNC(r.inventory_qty / r.ord_unit_qty) END)
                       , 0
                       , (CASE WHEN r.inventory_qty < 0 THEN 0 ELSE r.inventory_qty - (TRUNC(r.inventory_qty / r.ord_unit_qty) * r.ord_unit_qty) END)
                       , ''
                       , (CASE WHEN r.inventory_qty < 0 THEN -1*r.inventory_qty ELSE 0 END)
                       , (CASE WHEN r.inventory_qty < 0 THEN 'Y' ELSE 'N' END)
                       , SYSDATE
                       , asUserId
                       , SYSDATE
                       , asUserId
         ...
         Exception When DUP_VAL_ON_INDEX Then
            ...
            UPDATE  SURV_STOCK_DT
               SET  ORD_UNIT_QTY   = r.ord_unit_qty
                 ...
                 ,  ORD_SURV_QTY   = (CASE WHEN r.inventory_qty < 0 THEN 0 ELSE TRUNC(r.inventory_qty / r.ord_unit_qty) END)
                 ,  SALE_SURV_QTY  = 0
                 ,  SURV_QTY       = (CASE WHEN r.inventory_qty < 0 THEN 0 ELSE r.inventory_qty - (TRUNC(r.inventory_qty / r.ord_unit_qty) * r.ord_unit_qty) END)
                 ,  ADJ_QTY        = (CASE WHEN r.inventory_qty < 0 THEN -1*r.inventory_qty ELSE 0 END)
                 ,  S_CONFIRM_YN   = (CASE WHEN r.inventory_qty < 0 THEN 'Y' ELSE 'N' END)
                 ,  UPD_USER       = asUserId
                 ,  UPD_DT         = SYSDATE
             WHERE COMP_CD   = asCompCd
               AND SURV_DT   = asSurvDt
      ...
      Exception When OTHERS Then
         ...
         asRetMsg := '[' || r.item_cd || '] [' || to_char(r.ord_unit_qty) || '] [' || to_char(r.sale_unit_qty) || '] [' || to_char(r.inventory_qty) || ']' ;
         RollBack;
         GoTo ErrRtn;
      End ;
