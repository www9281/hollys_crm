--------------------------------------------------------
--  DDL for Procedure SP_MENU_USER_MOBILE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MENU_USER_MOBILE" 
(
...
BEGIN
     ...
     IF( p_proc_fg = 'S' ) THEN
          BEGIN
               ...
               INSERT INTO W_MENU_USER_MOBILE (COMP_CD, USER_ID, MENU_CD, INST_DT, INST_USER, UPD_DT, UPD_USER)
               ...
               if( s_menu_user_ref = 0  AND s_menu_ref <> 0 ) THEN
                    SP_MENU_USER_MOBILE(p_comp_cd, p_user_id ,  s_menu_ref , 'S' , p_reg_emp , liv_msg_code , liv_msg_code );
               END IF;
          END;
     ...
     ELSE IF( p_proc_fg = 'D') THEN
          ...
          EXCEPTION WHEN NO_DATA_FOUND THEN
               DELETE  FROM W_MENU_USER_MOBILE
               WHERE   COMP_CD = p_comp_cd
               AND     MENU_CD = p_menu_cd
               AND     USER_ID = p_user_id;
          ...
          IF(s_menu_ref_delete_yn = 'Y') THEN
               SP_MENU_USER_MOBILE(p_comp_cd, p_user_id ,  s_menu_ref , 'D' , p_reg_emp, liv_msg_code , liv_msg_code);
          END IF;
        END IF;
     END IF;
