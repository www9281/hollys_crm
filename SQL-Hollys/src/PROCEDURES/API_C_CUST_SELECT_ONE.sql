--------------------------------------------------------
--  DDL for Procedure API_C_CUST_SELECT_ONE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."API_C_CUST_SELECT_ONE" (
...
BEGIN
    ...
    ELSE 
      ...
      IF N_CARD_ID IS NOT NULL THEN
        v_query_into := v_query_into || ' AND EXISTS (SELECT 1 FROM C_CARD WHERE COMP_CD = A.COMP_CD AND CUST_ID = A.CUST_ID AND CARD_ID = ''' || N_CARD_ID || ''')';
      END IF;
      ...
      IF v_cust_cnt > 0 THEN
        ...
        SELECT 
             ...
             , (CASE WHEN C.CUST_ID IS NOT NULL THEN (CASE WHEN C.STOR_CD IS NOT NULL THEN GET_STOR_NM(C.BRAND_CD,C.STOR_CD)||''점에서 ''||GET_ITEM_NM(C.ITEM_CD)||''을 이미 사용하셨습니다.'' ELSE GET_ITEM_NM(C.ITEM_CD)||''을 이미 사용하셨습니다.'' END)
                     ELSE ''사용가능합니다'' 
                END) AS BNFIT_MSG
             , (CASE WHEN C.CUST_ID IS NOT NULL THEN ''N''
                     ELSE ''Y'' 
                END) AS BENEFIT_USE_YN
             , (CASE WHEN C.ITEM_CD IS NOT NULL THEN GET_ITEM_NM(C.ITEM_CD)
                     ELSE ''''
                END) AS BENEFIT_ITEM_NM
             , B.CARD_ID
             , A.DI_STR
        FROM C_CUST A, C_CARD B, MEMBER_BNFIT C
        WHERE A.COMP_CD = ''' || P_COMP_CD || '''
          AND A.BRAND_CD = ''' || P_BRAND_CD || '''
          ...
          AND C.USE_DT(+) = TO_CHAR(SYSDATE, ''YYYYMMDD'')
        ';
      ...
      ELSE 
        ...
        SELECT 
             ...
             , B.CARD_ID
             , A.DI_STR
        FROM C_CUST_REST A, C_CARD B, MEMBER_BNFIT C
        WHERE A.COMP_CD = ''' || P_COMP_CD || '''
          AND A.BRAND_CD = ''' || P_BRAND_CD || '''
          AND A.COMP_CD = B.COMP_CD
          AND A.CUST_ID = B.CUST_ID
