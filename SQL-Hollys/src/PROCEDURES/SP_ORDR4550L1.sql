--------------------------------------------------------
--  DDL for Procedure SP_ORDR4550L1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_ORDR4550L1" /*주문 현황 - 점포별 주문구분별*/
(
...
--           ||  ls_sql_item  -- S_ITEM
    ...
    ls_sql_main :=
        ...
        ||  '        SUM(SUM(S.ORD_AMT+S.ORD_VAT))  OVER (PARTITION BY S.BRAND_CD, B.BRAND_NM,  S.STOR_CD, B.STOR_NM, CL.CODE_NM )   AS ORD_AMT_TTL,'/*요청금액*/
        ||  '        SUM(SUM(S.ORD_VAT))  OVER (PARTITION BY S.BRAND_CD, B.BRAND_NM,  S.STOR_CD, B.STOR_NM, CL.CODE_NM )   AS ORD_VAT_TTL,'/*요청VAT*/
        ||  '        SUM(SUM(S.ORD_CAMT+S.ORD_CVAT)) OVER (PARTITION BY S.BRAND_CD, B.BRAND_NM,  S.STOR_CD, B.STOR_NM, CL.CODE_NM )   AS ORD_CAMT_TTL,'/*확정금액*/
        ||  '        SUM(SUM(S.ORD_CVAT)) OVER (PARTITION BY S.BRAND_CD, B.BRAND_NM,  S.STOR_CD, B.STOR_NM, CL.CODE_NM )   AS ORD_CVAT_TTL,'/*확정VAT*/
        ...
        ||  '              AND ' ||  ls_sql_date
        ||  '              AND  D.ORD_QTY <> 0           '
        ||  '            GROUP BY H.COMP_CD, H.BRAND_CD, H.STOR_CD, H.ORD_SEQ, H.ORD_FG    '
        ||  '      )  S,'
        ||  '      S_STORE B,'
        ||         ls_sql_cm_00395 || ' CL '
        ...
        ||  '        S.ORD_FG,'  /*구분코드-HIDDEN*/
        ||  '        CL.CODE_NM '
        ||  'ORDER BY 1,2,3,4,5,6 ASC' /*영업조직-HIDDEN*/  ;
