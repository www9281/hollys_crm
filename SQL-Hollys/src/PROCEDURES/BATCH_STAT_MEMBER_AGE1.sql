--------------------------------------------------------
--  DDL for Procedure BATCH_STAT_MEMBER_AGE1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."BATCH_STAT_MEMBER_AGE1" (
...
BEGIN
  ...
  -- Description   :   연령대/등급 회원 현황 자료 생성...전체
  --                    월별, 분기별, 년도별
  -- ==========================================================================================
  ...
  IF PI_YYMM    IS NULL OR
    ...
    PO_RETC := 'NG';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '1.전체:::파라메터 오류', PO_RETC, V_RETC);
    RETURN;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
             ...
             FROM   (
                  ...
                  , (--전체회원,신규회원
                     SELECT COMP_CD
                      ...
                      GROUP BY COMP_CD,BRAND_CD,SSS_DIV,AGE_RANGE,LVL_CD
                    )       T1
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원
                     ...
                     FROM   (
                             ...
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_M
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,B.AGE_RANGE_M,B.LVL_CD_M
                    )       T2
                  ...
                  , (
                     ...
                     AND    A.STOR_CD  = B.STOR_CD(+)
                     AND    A.BRAND_CD = B.BRAND_CD(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV_M,B.AGE_RANGE_M,B.LVL_CD_M
                    )       T4
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '1-1-1.전체-월별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --1-2.월별_합
  ...
  IF PO_RETC IS NULL THEN
      ...
      USING (SELECT 'M'                            INFO_DIV1
             ...
             FROM   (
                  ...
                  , (--전체회원,신규회원,객수,조수,총구매수량,총구매금액-회원
                     ...
                     AND    STOR_TG   = '-'
                     AND    STOR_CD   = '-'
                     GROUP BY COMP_CD,BRAND_CD,AGE_RANGE,LVL_CD
                    )       T1
                  , (--구매회원
                     SELECT A.COMP_CD                                                       COMP_CD
                          , A.BRAND_CD                                                      BRAND_CD
                     ...
                     FROM   (
                             ...
                             AND    AA.GIFT_DIV = '0'
                             AND    AA.CUST_ID IS NOT NULL
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,B.AGE_RANGE_M,B.LVL_CD_M
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '1-1-2.전체-월별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
             ...
             FROM   (
                  ...
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_Q
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,B.AGE_RANGE_Q,B.LVL_CD_Q
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
                  ...
                  , (
                     ...
                     AND    A.STOR_CD  = B.STOR_CD(+)
                     AND    A.BRAND_CD = B.BRAND_CD(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV_Q,B.AGE_RANGE_Q,B.LVL_CD_Q
                    )       T4
             WHERE  T0.COMP_CD   = T1.COMP_CD(+)
             AND    T0.BRAND_CD  = T1.BRAND_CD(+)
             AND    T0.SSS_DIV   = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '1-2-1.전체-분기별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --2-2.분기별_합
  ...
  IF PO_RETC IS NULL THEN
      ...
      USING (SELECT 'Q'                            INFO_DIV1
             ...
             FROM   (
                  ...
                  , (--구매회원
                     ...
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,B.AGE_RANGE_Q,B.LVL_CD_Q
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
                     ...
                     GROUP BY COMP_CD,BRAND_CD,AGE_RANGE,LVL_CD,AGE_RANGE,LVL_CD
                    )       T3
             WHERE  T0.COMP_CD   = T1.COMP_CD(+)
             AND    T0.BRAND_CD  = T1.BRAND_CD(+)
             AND    T0.AGE_RANGE = T1.AGE_RANGE(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '1-2-2.전체-분기별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
             ...
             FROM   (
                  ...
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_Y
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,B.AGE_RANGE_Y,B.LVL_CD_Y
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
                  ...
                  , (
                     ...
                     AND    A.STOR_CD  = B.STOR_CD(+)
                     AND    A.BRAND_CD = B.BRAND_CD(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV_Y,B.AGE_RANGE_Y,B.LVL_CD_Y
                    )       T4
             WHERE  T0.COMP_CD   = T1.COMP_CD(+)
             AND    T0.BRAND_CD  = T1.BRAND_CD(+)
             AND    T0.SSS_DIV   = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '1-3-1.전체-년도별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --3-2.년도별_합
  ...
  IF PO_RETC IS NULL THEN
      ...
      USING (SELECT 'Y'                            INFO_DIV1
             ...
             FROM   (
                  ...
                  , (--구매회원
                     ...
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,B.AGE_RANGE_Y,B.LVL_CD_Y
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '1-3-2.전체-년도별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
    COMMIT; 
    PO_RETC := 'OK';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '1.전체:::'||V_YYMM, PO_RETC, V_RETC);
  END IF;

END BATCH_STAT_MEMBER_AGE1;
