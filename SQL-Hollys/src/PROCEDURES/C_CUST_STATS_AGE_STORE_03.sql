--------------------------------------------------------
--  DDL for Procedure C_CUST_STATS_AGE_STORE_03
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_STATS_AGE_STORE_03" (
...
BEGIN
    ...
    SELECT
          ...
          , GET_COMMON_CODE_NM('00590', CUST.SIDO_CD, 'KOR') AS DIV_TYPE1
          ...
          -- 회원방문횟수          
          , (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND GET_AGE_GROUP(A.CUST_AGE) = CUST.CUST_AGE_GROUP AND B.SIDO_CD = CUST.SIDO_CD AND SALE_YM <= CUST.SALE_YM) AS MSS_VISIT_CNT  -- 방문횟수(누적)
          , CASE WHEN CUST.TOT_CUST_CNT = 0 THEN 0 ELSE (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND GET_AGE_GROUP(A.CUST_AGE) = CUST.CUST_AGE_GROUP AND B.SIDO_CD = CUST.SIDO_CD AND SALE_YM <= CUST.SALE_YM) / CUST.TOT_CUST_CNT END AS MSS_VISIT_CNT_AVG  -- 회원당 방문횟수
    ...
    FROM  (
            ...
            GROUP BY CUST.CUST_AGE_GROUP, STO.SIDO_CD, L01.SALE_YM
          ) CUST
          ,(
            SELECT  
                  MSS.CUST_AGE_GROUP 
    ...
    AND    CUST.SIDO_CD = MSS.SIDO_CD (+)
    AND    CUST.SALE_YM  = MSS.SALE_YM (+)
    ORDER BY CUST.CUST_AGE_GROUP, CUST.SIDO_CD, CUST.SALE_YM;
    
END C_CUST_STATS_AGE_STORE_03;
