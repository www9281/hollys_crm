--------------------------------------------------------
--  DDL for Procedure SP_CUST1090M0
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_CUST1090M0" /* 회원정보관리 MMS 재전송 */
(
  PSV_COMP_CD     IN  VARCHAR2 ,                -- 회사코드
  PSV_LANG_CD     IN  VARCHAR2 ,                -- Language Code
  PSV_ORD_DT      IN  VARCHAR2 ,                -- 주문일자
  PSV_ORD_SEQ     IN  VARCHAR2 ,                -- 주문번호
  PSV_ITEM_SEQ    IN  VARCHAR2 ,                -- 아이템번호
  PSV_GIFT_SEQ    IN  VARCHAR2 ,                -- 카드순번
  PSV_SEND_DIV    IN  VARCHAR2 ,                -- 전송구분(1:MMS전송,2:MMS대량전송,3:이메일전송)
...
BEGIN
    ...
    /*** MMS 전송 테이블 조회 ***/
    ls_sql := ' 
                SELECT  MSGKEY, SUBJECT,  PHONE, CALLBACK, MSG, FILE_CNT, FILE_CNT_REAL, RSLT,
    ...
    /*** MMS 전송 테이블 작성 ***/
    LOOP 
        FETCH CUR_1 INTO MYREC.MSGKEY, MYREC.SUBJECT,  MYREC.PHONE, MYREC.CALLBACK, MYREC.MSG, MYREC.FILE_CNT, MYREC.FILE_CNT_REAL, MYREC.RSLT,
        ...
        -- 재전송인 경우 예약 전송이 있는 경우 예약 전송을 삭제 
        IF MYREC.RSLT IS NULL THEN
            DELETE FROM MMS_MSG
            WHERE  MSGKEY = PSV_MSGKEY;
    ...
    IF ll_prc_cnt > 0 THEN
        ...
        SET     GIFT_SEND_DT    = ls_sms_send_dtm,
                GIFT_SEND_STAT  = '2',
                GIFT_ERR_CD     = '0000',
                GIFT_ERR_MSG    = FC_GET_WORDPACK(PSV_LANG_CD, 'SMS_SENDING'),
                MSGKEY          = ll_msg_key
        WHERE   COMP_CD         = PSV_COMP_CD
        AND     ORD_DT          = PSV_ORD_DT
        AND     ORD_SEQ         = PSV_ORD_SEQ
