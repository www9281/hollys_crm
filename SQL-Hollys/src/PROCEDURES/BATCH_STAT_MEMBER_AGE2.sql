--------------------------------------------------------
--  DDL for Procedure BATCH_STAT_MEMBER_AGE2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."BATCH_STAT_MEMBER_AGE2" (
...
BEGIN
  ...
  -- Description   :   연령대/등급 회원 현황 자료 생성...가맹유형별
  --                    월별, 분기별, 년도별
  -- ==========================================================================================
  ...
  IF PI_YYMM    IS NULL OR
    ...
    PO_RETC := 'NG';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '2.가맹유형별:::파라메터 오류', PO_RETC, V_RETC);
    RETURN;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
    BEGIN
      ...
      USING (SELECT 'M'                            INFO_DIV1
             ...
             FROM   (
                     ...
                     GROUP BY A.COMP_CD,A.BRAND_CD,C.SSS_DIV,X.CODE_CD,Y.LVL_CD,B.STOR_TP
                    )       T0
                  , (--전체회원,신규회원
                     SELECT COMP_CD
                          , BRAND_CD
                      ...
                      GROUP BY COMP_CD,BRAND_CD,SSS_DIV,AGE_RANGE,LVL_CD,STOR_TP
                    )       T1
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원
                     ...
                     FROM   (
                             ...
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_M,BB.STOR_TP
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,B.AGE_RANGE_M,B.LVL_CD_M,A.STOR_TP
                    )       T2
                  ...
                  , (
                     ...
                     AND    A.STOR_CD  = B.STOR_CD(+)
                     AND    A.BRAND_CD = B.BRAND_CD(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV_M,B.AGE_RANGE_M,B.LVL_CD_M,A.STOR_TP
                    )       T4
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '2-1-1.가맹유형별-월별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --1-2.월별_합
  ...
  IF PO_RETC IS NULL THEN
    BEGIN
      ...
      USING (SELECT 'M'                            INFO_DIV1
             ...
             FROM   (
                     ...
                     AND    X.CODE_TP  = V_AGE_GROUP
                     AND    A.COMP_CD  = Y.COMP_CD
                     GROUP BY A.COMP_CD,A.BRAND_CD,X.CODE_CD,Y.LVL_CD,B.STOR_TP
                    )       T0
                  , (--전체회원,신규회원,객수,조수,총구매수량,총구매금액-회원
                     ...
                     AND    STOR_TG   = '-'
                     AND    STOR_CD   = '-'
                     GROUP BY COMP_CD,BRAND_CD,AGE_RANGE,LVL_CD,STOR_TP
                    )       T1
                  , (--구매회원
                     SELECT A.COMP_CD                                                       COMP_CD
                          , A.BRAND_CD                                                      BRAND_CD
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.STOR_TP
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,B.AGE_RANGE_M,B.LVL_CD_M,A.STOR_TP
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '2-1-2.가맹유형별-월별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
             ...
             FROM   (
                  ...
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_Q,BB.STOR_TP
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,B.AGE_RANGE_Q,B.LVL_CD_Q,A.STOR_TP
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
                  ...
                  , (
                     ...
                     AND    A.STOR_CD  = B.STOR_CD(+)
                     AND    A.BRAND_CD = B.BRAND_CD(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV_Q,B.AGE_RANGE_Q,B.LVL_CD_Q,A.STOR_TP
                    )       T4
             WHERE  T0.COMP_CD   = T1.COMP_CD(+)
             AND    T0.BRAND_CD  = T1.BRAND_CD(+)
             AND    T0.SSS_DIV   = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '2-2-1.가맹유형별-분기별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --2-2.분기별_합
  ...
  IF PO_RETC IS NULL THEN
      ...
      USING (SELECT 'Q'                            INFO_DIV1
             ...
             FROM   (
                  ...
                  , (--구매회원
                     ...
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,B.AGE_RANGE_Q,B.LVL_CD_Q,A.STOR_TP
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '2-2-2.가맹유형별-분기별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
             ...
             FROM   (
                  ...
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_Y,BB.STOR_TP
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,B.AGE_RANGE_Y,B.LVL_CD_Y,A.STOR_TP
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
                  ...
                  , (
                     ...
                     AND    A.STOR_CD  = B.STOR_CD(+)
                     AND    A.BRAND_CD = B.BRAND_CD(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV_Y,B.AGE_RANGE_Y,B.LVL_CD_Y,A.STOR_TP
                    )       T4
             WHERE  T0.COMP_CD   = T1.COMP_CD(+)
             AND    T0.BRAND_CD  = T1.BRAND_CD(+)
             AND    T0.SSS_DIV   = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '2-3-1.가맹유형별-년도별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --3-2.년도별_합
  ...
  IF PO_RETC IS NULL THEN
      ...
      USING (SELECT 'Y'                            INFO_DIV1
             ...
             FROM   (
                  ...
                  , (--구매회원
                     ...
                     WHERE  A.COMP_CD = B.COMP_CD(+)
                     AND    A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,B.AGE_RANGE_Y,B.LVL_CD_Y,A.STOR_TP
                    )       T2
                  , (--매장당 일평균 객수,조수,판매금액-회원
                     SELECT COMP_CD
                          , BRAND_CD
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '2-3-2.가맹유형별-년도별_합:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
    COMMIT; 
    PO_RETC := 'OK';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_AGE1', '연령대/등급 회원 현황 자료 생성', '2.가맹유형별:::'||V_YYMM, PO_RETC, V_RETC);
  END IF;

END BATCH_STAT_MEMBER_AGE2;
