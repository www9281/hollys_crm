--------------------------------------------------------
--  DDL for Procedure SP_ORDR4550L6
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_ORDR4550L6" /*주문 현황 - 상품별*/
(
...
BEGIN
    ...
    ls_sql_main :=
        ...
        ||  '        I.ITEM_CD,' /*상품코드**/
        ||  '        I.ITEM_NM,' /*상품명*/
        ||  '        SUM(SUM(S.ORD_QTY))  OVER (PARTITION BY S.BRAND_CD, S.ORD_FG, I.ITEM_CD )   AS ORD_QTY_TTL,'/*요청수량*/
        ||  '        SUM(SUM(S.ORD_AMT+S.ORD_VAT))  OVER (PARTITION BY S.BRAND_CD, S.ORD_FG, I.ITEM_CD )   AS ORD_AMT_TTL,'/*요청금액*/
        ||  '        SUM(SUM(S.ORD_VAT))  OVER (PARTITION BY S.BRAND_CD, S.ORD_FG, I.ITEM_CD )   AS ORD_VAT_TTL,'/*요청VAT*/
        ||  '        SUM(SUM(S.ORD_CQTY)) OVER (PARTITION BY S.BRAND_CD, S.ORD_FG, I.ITEM_CD )   AS ORD_CQTY_TTL,'/*확정수량*/
        ||  '        SUM(SUM(S.ORD_CAMT+S.ORD_CVAT)) OVER (PARTITION BY S.BRAND_CD, S.ORD_FG, I.ITEM_CD )   AS ORD_CAMT_TTL,'/*확정금액*/
        ||  '        SUM(SUM(S.ORD_CVAT)) OVER (PARTITION BY S.BRAND_CD, S.ORD_FG, I.ITEM_CD )   AS ORD_CVAT_TTL,'/*확정VAT*/
        ...
        ||  '        I.M_CLASS_NM, ' /*중분류명*/
        ||  '        I.S_CLASS_CD, ' /*소분류-HIDDEN**/
        ||  '        I.S_CLASS_NM, I.ITEM_CD, I.ITEM_NM ' /*소분류명*/
        ||  'ORDER BY 1,2,3,4,5,6,7,8,9 , 14 ASC' /*영업조직-HIDDEN*/  ;
