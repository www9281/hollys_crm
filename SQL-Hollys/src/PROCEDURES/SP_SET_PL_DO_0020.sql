--------------------------------------------------------
--  DDL for Procedure SP_SET_PL_DO_0020
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_SET_PL_DO_0020" ( PSV_COMP_CD  IN VARCHAR2,  -- 회사코드
    ...
    CURSOR CUR_1 IS
       ...
       (
            SELECT  ITC.COMP_CD
                  ...
                  , ITC.STOR_TP
                  , ITC.ITEM_CD
                  , ITC.L_CLASS_CD      AS CLASS_CD
                  , ITC.ORD_SALE_DIV
                  , NVL(ICH.SALE_PRC, ITC.SALE_PRC) SALE_PRC
                  , ROUND(NVL(ICH.COST,     ITC.COST) / ITC.ORD_UNIT_QTY, 0) COST
                  , ITC.SALE_VAT_YN
            ...
            FROM   (      
                    SELECT  /*+ LEADING(STO) INDEX(STO PK_STORE) */
                          ...
                          , NVL((
                                 SELECT  L_CLASS_CD
                                 FROM    ITEM ZZ
                                 WHERE   ZZ.COMP_CD = RCP.COMP_CD
                                 AND     ZZ.ITEM_CD = RCP.P_ITEM_CD
                                 ), ICL.L_CLASS_CD) L_CLASS_CD 
                          , ITC.ORD_SALE_DIV
                          , ITC.SALE_PRC
                          , ITC.COST
                          , ITC.SALE_VAT_YN
                    ...
                    FROM    STORE                   STO
                          ...
                          ,(
                            SELECT  COMP_CD,
                                    ...
                                    P_ITEM_CD,
                                    C_ITEM_CD,
                                    ROW_NUMBER() OVER(PARTITION BY COMP_CD, BRAND_CD, C_ITEM_CD ORDER BY P_ITEM_CD) R_NUM
                    ...
                    AND     ITC.COMP_CD  = RCP.COMP_CD  (+)
                    AND     ITC.BRAND_CD = RCP.BRAND_CD (+)
                    AND     ITC.ITEM_CD  = RCP.C_ITEM_CD(+)
                    AND     1            = RCP.R_NUM    (+)
                  ...
                  ,(
                    SELECT  /*+ LEADING(STO) INDEX(STO PK_STORE) */
                          ...
                          , ICH.COST
                          , ROW_NUMBER()
                                OVER(PARTITION BY STO.COMP_CD, STO.BRAND_CD, STO.STOR_CD, STO.STOR_TP, ICH.ITEM_CD
                                     ORDER BY ICH.START_DT DESC) R_NUM
                    FROM    STORE                   STO
                          , ITEM_CHAIN_HIS          ICH
                    WHERE   STO.COMP_CD   = ICH.COMP_CD
                    AND     STO.BRAND_CD  = ICH.BRAND_CD
                    AND     STO.STOR_TP   = ICH.STOR_TP
                    AND     STO.COMP_CD   = PSV_COMP_CD
                    AND     STO.BRAND_CD  = PSV_BRAND_CD
                    AND     STO.STOR_CD   = PSV_STOR_CD
                    AND     ICH.START_DT <= PSV_STD_YM||'31'
                   ) ICH
            WHERE   ITC.COMP_CD  = ICH.COMP_CD (+)
            AND     ITC.BRAND_CD = ICH.BRAND_CD(+)
            AND     ITC.STOR_CD  = ICH.STOR_CD (+)
               ...
               (
                SELECT  ITM.COMP_CD
                      ...
                      , CASE WHEN ITM.SALE_VAT_YN = 'Y' THEN (JDM.GRD_AMT - JDM.VAT_AMT) + CEIL((JDM.DC_AMT + JDM.ENR_AMT) / (1 + ITM.SALE_VAT_IN_RATE))
                             ELSE (JDM.GRD_AMT - JDM.VAT_AMT) END  AS SAL_N_AMT
                      , 0                           AS LST_M_STK
                      , 0                           AS CUR_M_BUY
                      , 0                           AS CUR_M_STK
