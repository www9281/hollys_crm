--------------------------------------------------------
--  DDL for Procedure SP_MEAN2040L0_2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MEAN2040L0_2" /* 회원등급/상품분류별 구매분석 */
(
...
/* 가로축 데이타 FETCH */
   ls_sql_crosstab_main :=
    ...
    /* MAIN SQL */
    ls_sql_main :=                  '   SELECT  V02.LVL_CD          '
                ||chr(13)||chr(10)||'         , CASE WHEN '''||PSV_SORT_DIV||''' = ''01'' THEN RANK_OF_QTY ELSE RANK_OF_AMT END AS ITM_RANK             '                     
                ...
                ||chr(13)||chr(10)||'                 , SUM(V01.GRD_AMT) OVER(PARTITION BY V01.COMP_CD, V01.LVL_CD                       ) AS AGE_SUM_GRD  '
                ||chr(13)||chr(10)||'                 , ROW_NUMBER()     OVER(PARTITION BY V01.COMP_CD, V01.LVL_CD  ORDER BY SALE_QTY DESC) AS RANK_OF_QTY '
                ||chr(13)||chr(10)||'                 , ROW_NUMBER()     OVER(PARTITION BY V01.COMP_CD, V01.LVL_CD  ORDER BY GRD_AMT  DESC) AS RANK_OF_AMT '
                ||chr(13)||chr(10)||'           FROM   (                                                '
