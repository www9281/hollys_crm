--------------------------------------------------------
--  DDL for Procedure C_CUST_USER_ANALYSIS_COMPARE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_USER_ANALYSIS_COMPARE" (
...
--------------------------------- 전체 회원 분석 대비 ----------------------------------
    v_query := 
              'SELECT  CU.CUST_ID
                 ...
                 ,  SUB.PNT_RANK                AS  PNT_RANK
                 ,  COP.SAV_PT                 AS  C_PNT_SUM
                 ,  COP.PNT_RANK                AS  C_PNT_RANK
              ...
              FROM  C_CUST CU
                 ,  (
                        SELECT  CU.COMP_CD
                             ...
                             ,  CSH.SAV_PT
                             ,  RANK () OVER (ORDER BY CSH.SAV_PT DESC NULLS LAST)                 AS  PNT_RANK
                             ,  DMS.SALE_QTY
                             ,  DMS.GRD_AMT
                 ...
                 ,  (
                        SELECT  CU.COMP_CD
                          ...
                          FROM  (
                                    SELECT  COMP_CD,  CUST_ID
                                         ,  TO_DATE(''' || P_LAST_END_DT || ''', ''YYYYMMDD'') - TO_DATE(JOIN_DT, ''YYYYMMDD'') + 1  AS  DIFF
                                         ,  CASE WHEN JOIN_DT < ''' || P_LAST_START_DT || ''' THEN TO_DATE(''' || P_LAST_END_DT || ''', ''YYYYMMDD'') - TO_DATE(''' || P_LAST_START_DT || ''', ''YYYYMMDD'')
                                                 ELSE TO_DATE(''' || P_LAST_END_DT || ''', ''YYYYMMDD'') - TO_DATE(JOIN_DT, ''YYYYMMDD'')
                                                 END + 1                                                    AS  DIFF_RANGE
                                      FROM  C_CUST  CU
                                     WHERE  CU.COMP_CD  =  ''' || P_COMP_CD || '''
                                       AND  ( ''' || N_CUST_STAT || ''' IS NULL OR CU.CUST_STAT =  ''' || N_CUST_STAT || ''')
...
END C_CUST_USER_ANALYSIS_COMPARE;
