--------------------------------------------------------
--  DDL for Procedure SP_MEAN2020L0
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_MEAN2020L0" /* 회원등급/상품분류별 구매분석 */
(
...
/* 가로축 데이타 FETCH */
   ls_sql_crosstab_main :=
    ...
    ls_sql_main :=                  '   SELECT  V02.L_CLASS_NM      '
                ...
                ||chr(13)||chr(10)||'                 , SUM(V01.LVL_SUM_GRD) OVER(PARTITION BY V01.COMP_CD, V01.L_CLASS_CD, V01.M_CLASS_CD, V01.S_CLASS_CD) CLS_SUM_GRD  '
                ||chr(13)||chr(10)||'           FROM   (               '
