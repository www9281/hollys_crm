--------------------------------------------------------
--  DDL for Procedure BATCH_STAT_MEMBER_TOT7
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."BATCH_STAT_MEMBER_TOT7" (
...
BEGIN
  ...
  -- Description   :   전체 회원 현황 자료 생성...매장별
  --                    월별, 분기별, 년도별
  -- ==========================================================================================
  ...
  IF PI_YYMM    IS NULL OR
    ...
    PO_RETC := 'NG';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT7', '전체 회원 현황 자료 생성', '7.매장별:::파라메터 오류', PO_RETC, V_RETC);
    RETURN;
  END IF;
  ...
  IF PO_RETC IS NULL THEN
    BEGIN
      ...
      USING (SELECT 'M'                            INFO_DIV1
             ...
             FROM   (
                     ...
                     GROUP BY A.COMP_CD,A.BRAND_CD,C.SSS_DIV,B.STOR_CD
                    )       T0
                  , (--전체회원,신규회원
                     SELECT COMP_CD
                          , BRAND_CD
                     ...
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV,STOR_CD
                    )       T1
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     FROM   (
                             ...
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_M,BB.STOR_CD
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,A.STOR_CD
                    )       T2
                  ...
                  , (
                     ...
                     FROM   TEMP_STORE
                     WHERE  COMP_CD = V_COMP_CD
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV_M,STOR_CD
                    )       T4
             WHERE  T0.COMP_CD  = T1.COMP_CD(+)
             AND    T0.BRAND_CD = T1.BRAND_CD(+)
             AND    T0.SSS_DIV  = T1.SSS_DIV(+)
             AND    T0.STOR_CD  = T1.STOR_CD(+)
             AND    T0.COMP_CD  = T2.COMP_CD(+)
             AND    T0.BRAND_CD = T2.BRAND_CD(+)
             AND    T0.SSS_DIV  = T2.SSS_DIV(+)
             AND    T0.STOR_CD  = T2.STOR_CD(+)
             AND    T0.COMP_CD  = T3.COMP_CD(+)
             AND    T0.BRAND_CD = T3.BRAND_CD(+)
             AND    T0.SSS_DIV  = T3.SSS_DIV(+)
             AND    T0.STOR_CD  = T3.STOR_CD(+)
             AND    T0.COMP_CD  = T4.COMP_CD(+)
             AND    T0.BRAND_CD = T4.BRAND_CD(+)
             AND    T0.SSS_DIV  = T4.SSS_DIV(+)
             AND    T0.STOR_CD  = T4.STOR_CD(+)
             AND   (   T1.MEMB_TOT          > 0
                    OR T1.MEMB_NEW_AAPP     > 0
                    OR T1.MEMB_NEW_UAPP     > 0
                    OR T2.MEMB_SALE_AAPP    > 0
                    OR T2.MEMB_SALE_UAPP    > 0
                    OR T2.MEMB_CUST_CNT     > 0
                    OR T2.PUBL_CUST_CNT     > 0
                    OR T2.MEMB_BILL_CNT     > 0
                    OR T2.PUBL_BILL_CNT     > 0
                    OR T2.MEMB_TOT_SALE_QTY > 0
                    OR T2.PUBL_TOT_SALE_QTY > 0
                    OR T2.MEMB_TOT_SALE_AMT > 0
                    OR T2.PUBL_TOT_SALE_AMT > 0
                    OR T3.MEMB_CUST_CNT     > 0
                    OR T3.PUBL_CUST_CNT     > 0
                    OR T3.MEMB_BILL_CNT     > 0
                    OR T3.PUBL_BILL_CNT     > 0
                    OR T3.MEMB_SALE_AMT     > 0
                    OR T3.PUBL_SALE_AMT     > 0)
            )                  SOC
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT7', '전체 회원 현황 자료 생성', '7-1-1.매장별-월별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --1-2.월별_합
  ...
  IF (PO_RETC IS NULL) AND (PI_QTFV = TRUE) THEN
             ...
             FROM   (
                  ...
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_Q,BB.STOR_CD
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,A.STOR_CD
                  ...
                  , (
                     ...
                     FROM   TEMP_STORE
                     WHERE  COMP_CD = V_COMP_CD
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV_Q,STOR_CD
                    )       T4
             WHERE  T0.COMP_CD  = T1.COMP_CD(+)
             AND    T0.BRAND_CD = T1.BRAND_CD(+)
             AND    T0.SSS_DIV  = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT7', '전체 회원 현황 자료 생성', '7-2-1.매장별-분기별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  --------------------------------------------------------------------------------------------------
  --2-2.분기별_합
  ...
  IF (PO_RETC IS NULL) AND (PI_YTFV = TRUE) THEN
             ...
             FROM   (
                  ...
                  , (--구매회원,객수,조수,총구매수량,총구매금액-회원/비회원
                     ...
                     FROM   (
                             ...
                             AND    AA.BRAND_CD = BB.BRAND_CD(+)
                             AND    AA.STOR_CD  = BB.STOR_CD(+)
                             GROUP BY AA.COMP_CD,AA.BRAND_CD,AA.CUST_ID,BB.SSS_DIV_Y,BB.STOR_CD
                            )               A
                          , TEMP_CUST_STAT1 B
                     WHERE  A.CUST_ID = B.CUST_ID(+)
                     GROUP BY A.COMP_CD,A.BRAND_CD,A.SSS_DIV,A.STOR_CD
                  ...
                  , (
                     ...
                     FROM   TEMP_STORE
                     WHERE  COMP_CD = V_COMP_CD
                     GROUP BY COMP_CD,BRAND_CD,SSS_DIV_Y,STOR_CD
                    )       T4
             WHERE  T0.COMP_CD  = T1.COMP_CD(+)
             AND    T0.BRAND_CD = T1.BRAND_CD(+)
             AND    T0.SSS_DIV  = T1.SSS_DIV(+)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ROLLBACK;
        PO_RETC := 'NG';
        STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT7', '전체 회원 현황 자료 생성', '7-3-1.매장별-년도별_SSS구분별:::'||SQLERRM, PO_RETC, V_RETC);
    END;
  END IF;
  ---------------------------------------------------------------------------------------------------
  --3-2.년도별_합
  ...
  IF PO_RETC IS NULL THEN
    COMMIT;
    PO_RETC := 'OK';
    STAT_LOG_SAVE('BATCH_STAT_MEMBER_TOT7', '전체 회원 현황 자료 생성', '7.매장별:::'||V_YYMM, PO_RETC, V_RETC);
  END IF;

END BATCH_STAT_MEMBER_TOT7;
