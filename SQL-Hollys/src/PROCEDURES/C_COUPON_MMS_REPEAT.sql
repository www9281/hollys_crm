--------------------------------------------------------
--  DDL for Procedure C_COUPON_MMS_REPEAT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_COUPON_MMS_REPEAT" /* 회원정보관리 MMS 재전송 */
(
  P_PSV_COMP_CD     IN  VARCHAR2 ,                -- 회사코드
  P_PSV_COUPON_CD   IN  VARCHAR2 ,                -- 쿠폰번호
  P_PSV_CERT_NO     IN  VARCHAR2 ,                -- 인증번호
  P_GIFT_SEND_DT    IN  VARCHAR2 ,                -- 전송일시
  P_PSV_MSGKEY      IN  VARCHAR2 ,                -- MMS 키 값
...
******************************************************************************/
    ...
    ls_tbl_name     VARCHAR2(2000) := 'MMS_LOG_'||SUBSTR(REPLACE(P_GIFT_SEND_DT, '/', ''), 1, 6);     -- 테이블 이름
...
BEGIN
    ...
    IF ll_prc_cnt > 0 THEN
        ...
        SET     GIFT_SEND_DT    = ls_sms_send_dtm,
                GIFT_SEND_STAT  = '2',
                GIFT_ERR_CD     = '0000',
                GIFT_ERR_MSG    = FC_GET_WORDPACK('KOR', 'SMS_SENDING'),
                MSGKEY          = ll_msg_key
        WHERE   COMP_CD   = P_PSV_COMP_CD
        AND     COUPON_CD = P_PSV_COUPON_CD
        AND     CERT_NO   = P_PSV_CERT_NO;
    END IF;
    
    O_PR_RTN_CD  := ls_err_cd;
    O_PR_RTN_MSG := ls_err_msg ;
    /*
    OPEN PR_RESULT FOR
        SELECT 0 FROM DUAL;
    */    
