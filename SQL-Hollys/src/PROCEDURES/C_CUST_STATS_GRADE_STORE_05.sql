--------------------------------------------------------
--  DDL for Procedure C_CUST_STATS_GRADE_STORE_05
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_STATS_GRADE_STORE_05" (
...
BEGIN
    ...
    SELECT
          ...
          -- 회원방문횟수          
          , (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND A.CUST_LVL = CUST.LVL_CD AND B.STOR_TG = CUST.STOR_TG AND SALE_YM <= CUST.SALE_YM) AS MSS_VISIT_CNT  -- 방문횟수(누적)
          , CASE WHEN CUST.TOT_CUST_CNT = 0 THEN 0 ELSE (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND A.CUST_LVL = CUST.LVL_CD AND B.STOR_TG = CUST.STOR_TG AND SALE_YM <= CUST.SALE_YM) / CUST.TOT_CUST_CNT END AS MSS_VISIT_CNT_AVG  -- 회원당 방문횟수
    ...
    FROM  (
            ...
            GROUP BY CUST.LVL_CD, STO.STOR_TG, L01.SALE_YM
          ) CUST
          ,(
            SELECT  
                  MSS.CUST_LVL
    ...
    ORDER BY CUST.LVL_CD, CUST.STOR_TG, CUST.SALE_YM;
    
END C_CUST_STATS_GRADE_STORE_05;
