--------------------------------------------------------
--  DDL for Procedure SP_ORDR4820L2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."SP_ORDR4820L2" /* 주문확정현황_요일별 */
(
...
BEGIN
    ...
    ls_sql_crosstab_main :=
         '  SELECT   H.SHIP_DT   '
    ||   '       ,   SUBSTR(H.SHIP_DT, 7, 2) AS SHIP_DD  '
    ||  Q'[      ,   FC_GET_WEEK(']' || PSV_COMP_CD || q'[', H.SHIP_DT, ']' || PSV_LANG_CD || q'[')   AS SHIP_DY  ]'
    ||   '    FROM   ORDER_HD H  '
    ||   '       ,   S_STORE  S  '
    ...
    LOOP
        BEGIN
            IF i > 1 THEN
               ...
               V_HD := V_HD || ' , ' ;
            END IF;
            V_CROSSTAB := V_CROSSTAB || '''' || qry_hd(i).SHIP_DT || '''';
            V_HD := V_HD || '''' || qry_hd(i).SHIP_DD || '.' ||  qry_hd(i).SHIP_DY || '''' ;
        END;
    END LOOP;
    ...
    OPEN PR_RESULT FOR V_SQL USING PSV_COMP_CD, PSV_ITEM_GRP, PSV_ITEM_GRP, PSV_ITEM_TP, PSV_ITEM_TP;
