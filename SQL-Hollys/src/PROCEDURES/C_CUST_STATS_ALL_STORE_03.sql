--------------------------------------------------------
--  DDL for Procedure C_CUST_STATS_ALL_STORE_03
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_STATS_ALL_STORE_03" (
...
BEGIN
    ...
    -- Description   :   회원분석 전체회원 현황 [지역별 탭]
    -- ==========================================================================================
    OPEN O_CURSOR FOR
    SELECT 
          ...
          -- 회원방문횟수          
          , (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND B.SIDO_CD = CUST.SIDO_CD AND SALE_YM <= CUST.SALE_YM) AS MSS_VISIT_CNT  -- 방문횟수(누적)
          , CASE WHEN CUST.TOT_CUST_CNT = 0 THEN 0 ELSE (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND B.SIDO_CD = CUST.SIDO_CD AND SALE_YM <= CUST.SALE_YM) / CUST.TOT_CUST_CNT END AS MSS_VISIT_CNT_AVG  -- 회원당 방문횟수
    ...
    FROM  (
            ...
            GROUP BY STO.SIDO_CD, L01.SALE_YM
          ) CUST
          ,(
            SELECT  
                  MSS.SIDO_CD
    ...
    WHERE  CUST.SIDO_CD        = JDS.SIDO_CD
    AND    CUST.SALE_YM        = JDS.SALE_YM
    AND    JDS.SIDO_CD         = MSS.SIDO_CD (+)
    AND    JDS.SALE_YM         = MSS.SALE_YM (+)
    ORDER BY CUST.SIDO_CD, JDS.SALE_YM;
    
END C_CUST_STATS_ALL_STORE_03;
