--------------------------------------------------------
--  DDL for Procedure C_CUST_STATS_AGE_STORE_02
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "CRMDEV"."C_CUST_STATS_AGE_STORE_02" (
...
BEGIN
    ...
    -- Description   :   회원분석 연령대 현황 [상권별 탭]
    -- ==========================================================================================
    OPEN O_CURSOR FOR
    SELECT
          GET_COMMON_CODE_NM('01760', CUST.CUST_AGE_GROUP, 'KOR') AS AGE_GROUP
          , GET_COMMON_CODE_NM('00595', CUST.TRAD_AREA, 'KOR') AS DIV_TYPE1
          ...
          -- 회원방문횟수          
          , (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND GET_AGE_GROUP(A.CUST_AGE) = CUST.CUST_AGE_GROUP AND B.TRAD_AREA = CUST.TRAD_AREA AND SALE_YM <= CUST.SALE_YM) AS MSS_VISIT_CNT  -- 방문횟수(누적)
          , CASE WHEN CUST.TOT_CUST_CNT = 0 THEN 0 ELSE (SELECT SUM(BILL_CNT) FROM C_CUST_MSS A, STORE B WHERE A.STOR_CD = B.STOR_CD AND GET_AGE_GROUP(A.CUST_AGE) = CUST.CUST_AGE_GROUP AND B.TRAD_AREA = CUST.TRAD_AREA AND SALE_YM <= CUST.SALE_YM) / CUST.TOT_CUST_CNT END AS MSS_VISIT_CNT_AVG  -- 회원당 방문횟수
    ...
    FROM  (
            ...
            GROUP BY CUST.CUST_AGE_GROUP, STO.TRAD_AREA, L01.SALE_YM
          ) CUST
          ,(
            SELECT  
                  MSS.CUST_AGE_GROUP 
    ...
    AND    CUST.TRAD_AREA = MSS.TRAD_AREA (+)
    AND    CUST.SALE_YM  = MSS.SALE_YM (+)
    ORDER BY CUST.CUST_AGE_GROUP, CUST.TRAD_AREA, CUST.SALE_YM;
    
END C_CUST_STATS_AGE_STORE_02;
