--------------------------------------------------------
--  DDL for Package Body PKG_SALE4171
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "CRMDEV"."PKG_SALE4171" AS
    ...
    ******************************************************************************/
    ...
    BEGIN
        ...
        /* 가로축 데이타 FETCH */
        ls_sql_tab_main := ''
        ||CHR(13)||CHR(10)||Q'[ SELECT  DISTINCT SUBSTR(SJ.SALE_DT, 1, 6) AS SALE_YM, SUBSTR(SJ.SALE_DT, 1, 4) || '-' || SUBSTR(SJ.SALE_DT, 5, 2)   AS SALE_YM_NM ]'
        ...
        ||CHR(13)||CHR(10)||Q'[  ORDER  BY SUBSTR(SJ.SALE_DT, 1, 6) ]' || PSV_ORDER_BY;
        ...
        LOOP
            BEGIN
                ...
                V_HD1 := V_HD1 || CHR(13) || CHR(10) || Q'[      ,  ']' || qry_hd(i).SALE_YM_NM  || Q'[' AS CT]' || TO_CHAR(i*3 - 2);
                V_HD1 := V_HD1 || CHR(13) || CHR(10) || Q'[      ,  ']' || qry_hd(i).SALE_YM_NM  || Q'[' AS CT]' || TO_CHAR(i*3 - 1);
                V_HD1 := V_HD1 || CHR(13) || CHR(10) || Q'[      ,  ']' || qry_hd(i).SALE_YM_NM  || Q'[' AS CT]' || TO_CHAR(i*3);
        ...
        ||CHR(13)||CHR(10)||Q'[      ,  SUM(SUM(DECODE(:PSV_CUST_DIV, 'C', SJ.ETC_M_CNT + SJ.ETC_F_CNT, SJ.BILL_CNT - SJ.R_BILL_CNT))) OVER (PARTITION BY SJ.BRAND_CD, SJ.STOR_CD)    AS TOT_CUST_CNT ]'
        ||CHR(13)||CHR(10)||Q'[      ,  DECODE(SUM(SUM(DECODE(:PSV_CUST_DIV, 'C', SJ.ETC_M_CNT + SJ.ETC_F_CNT, SJ.BILL_CNT - SJ.R_BILL_CNT))) OVER (PARTITION BY SJ.BRAND_CD, SJ.STOR_CD), 0, 0, SUM(SUM(DECODE(:PSV_FILTER, 'G', SJ.GRD_AMT, SJ.GRD_AMT - SJ.VAT_AMT))) OVER (PARTITION BY SJ.BRAND_CD, SJ.STOR_CD) / SUM(SUM(DECODE(:PSV_CUST_DIV, 'C', SJ.ETC_M_CNT + SJ.ETC_F_CNT, SJ.BILL_CNT - SJ.R_BILL_CNT))) OVER (PARTITION BY SJ.BRAND_CD, SJ.STOR_CD))  AS TOT_CUST_AMT ]'
        ||CHR(13)||CHR(10)||Q'[      ,  SUBSTR(SJ.SALE_DT, 1, 6)    AS SALE_YM  ]'
        ...
        ||CHR(13)||CHR(10)||Q'[      ,  DECODE(SUM(DECODE(:PSV_CUST_DIV, 'C', SJ.ETC_M_CNT + SJ.ETC_F_CNT, SJ.BILL_CNT - SJ.R_BILL_CNT)), 0, 0, SUM(DECODE(:PSV_FILTER, 'G', SJ.GRD_AMT, SJ.GRD_AMT - SJ.VAT_AMT)) / SUM(DECODE(:PSV_CUST_DIV, 'C', SJ.ETC_M_CNT + SJ.ETC_F_CNT, SJ.BILL_CNT - SJ.R_BILL_CNT))) AS CUST_AMT ]'
        ||CHR(13)||CHR(10)||Q'[   FROM  SALE_JDS    SJ  ]'
        ||CHR(13)||CHR(10)||Q'[      ,  S_STORE     S   ]'
        ||CHR(13)||CHR(10)||Q'[  WHERE  SJ.COMP_CD  = S.COMP_CD     ]'
        ||CHR(13)||CHR(10)||Q'[    AND  SJ.BRAND_CD = S.BRAND_CD    ]'
        ...
        ||CHR(13)||CHR(10)||Q'[      ,  SUBSTR(SJ.SALE_DT, 1, 6)    ]'
        ||CHR(13)||CHR(10)||Q'[      ,  SJ.STOR_CD      ]';
