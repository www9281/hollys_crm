--------------------------------------------------------
--  DDL for Package Body PKG_SALE1110
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "CRMDEV"."PKG_SALE1110" AS
    ...
    ******************************************************************************/
    ...
    BEGIN
        ...
        ||CHR(13)||CHR(10)||Q'[             SELECT  I.ITEM_CD AS ITEM_CD]'
        ||CHR(13)||CHR(10)||Q'[                  ,  I.ITEM_NM AS ITEM_NM]'
        ||CHR(13)||CHR(10)||Q'[                  ,  SUM(SD.SALE_QTY) OVER (PARTITION BY SD.COMP_CD, I.ITEM_CD) AS TOT_QTY  ]'
        ||CHR(13)||CHR(10)||Q'[                  ,  SUM(DECODE(:PSV_FILTER, 'G', SD.GRD_AMT, 'N', SD.GRD_AMT - SD.VAT_AMT, SD.SALE_AMT)) OVER (PARTITION BY SD.COMP_CD, I.ITEM_CD) AS TOT_AMT ]'
