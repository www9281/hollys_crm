--------------------------------------------------------
--  DDL for Package Body PKG_SALE4700
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "CRMDEV"."PKG_SALE4700" AS
   PROCEDURE SP_MAIN
    ( 
    ...
    ******************************************************************************/
    ...
    BEGIN
        ...
        ||CHR(13)||CHR(10)||Q'[                              ,  SUM(DECODE(:PSV_CUST_DIV, 'C', SJ.ETC_M_CNT + SJ.ETC_F_CNT, SJ.BILL_CNT - SJ.R_BILL_CNT)) OVER (PARTITION BY SJ.SALE_DT)  AS CUST_CNT ]'
        ||CHR(13)||CHR(10)||Q'[                              ,  SUM(SJ.SALE_QTY) OVER (PARTITION BY SJ.SALE_DT)     AS SALE_QTY ]'
        ||CHR(13)||CHR(10)||Q'[                              ,  ROUND(DECODE(SUM(DECODE(:PSV_CUST_DIV, 'C', SJ.ETC_M_CNT + SJ.ETC_F_CNT, SJ.BILL_CNT - SJ.R_BILL_CNT)) OVER (PARTITION BY SJ.SALE_DT), 0, 0,  ]'
        ||CHR(13)||CHR(10)||Q'[                                              SUM(DECODE(:PSV_FILTER, 'T', SJ.SALE_AMT, 'G', SJ.GRD_AMT, SJ.GRD_AMT - SJ.VAT_AMT)) OVER (PARTITION BY SJ.SALE_DT)                ]'
        ||CHR(13)||CHR(10)||Q'[                                              / SUM(DECODE(:PSV_CUST_DIV, 'C', SJ.ETC_M_CNT + SJ.ETC_F_CNT, SJ.BILL_CNT - SJ.R_BILL_CNT)) OVER (PARTITION BY SJ.SALE_DT)         ]'
    ...
    ******************************************************************************/
    ...
    BEGIN
        ...
        ||CHR(13)||CHR(10)||Q'[          FROM ( ]'
        ||CHR(13)||CHR(10)||Q'[               SELECT SALE_DT ]'
        ||CHR(13)||CHR(10)||Q'[                    , SUM(DECODE(:PSV_CUST_DIV, 'C', S.ETC_M_CNT + S.ETC_F_CNT, S.BILL_CNT - S.R_BILL_CNT)) OVER(PARTITION BY S.SALE_DT) AS CUST_CNT1 ]' -- 고객수
        ...
        ||CHR(13)||CHR(10)||Q'[                    , SUM(DECODE(:PSV_CUST_DIV, 'C', S.ETC_M_CNT + S.ETC_F_CNT, S.BILL_CNT - S.R_BILL_CNT)) OVER(PARTITION BY S.SALE_DT) AS CUST_CNT2 ]' -- 고객수
        ...
        ||CHR(13)||CHR(10)||Q'[                    , TO_NUMBER(NULL) AS CUST_CNT2  ]'
        ||CHR(13)||CHR(10)||Q'[                    , TO_NUMBER(NULL) AS SALE_AMT2  ]'
        ||CHR(13)||CHR(10)||Q'[                    , SUM(DECODE(:PSV_CUST_DIV, 'C', S.ETC_M_CNT + S.ETC_F_CNT, S.BILL_CNT - S.R_BILL_CNT)) AS CUST_CNT3 ]' -- 고객수
