--------------------------------------------------------
--  DDL for Package Body PKG_SALE4030
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "CRMDEV"."PKG_SALE4030" AS
       ...
       ls_sql_crosstab_main := ''
        ...
        LOOP
            BEGIN
                ...
                V_HD2 := V_HD2 || ','''   || FC_GET_WORDPACK(PSV_COMP_CD, PSV_LANG_CD, 'TIME_SALE_AMT_TAB')  ||  ''' CT' || TO_CHAR(i) ;
            END;
        END LOOP;
       ...
       ls_sql_crosstab_main := ''
        ...
        LOOP
            BEGIN
                ...
                --V_HD1 := V_HD1 || ' V04  CT' || TO_CHAR(i)  ;
                V_HD1 := V_HD1 || ''''   || qry_hd(i).SEC_DIV_NM  ||  ''' CT' || TO_CHAR(i ) ;
                V_HD2 := V_HD2 || ','''   || FC_GET_WORDPACK(PSV_COMP_CD, PSV_LANG_CD, 'TIME_QTY_TAB') ||  ''' CT' || TO_CHAR(i) ;
            END;
        END LOOP;
