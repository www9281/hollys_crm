...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PACKAGE BODY "CRMDEV"."PKG_REPORT" AS
       ...
       CASE PSV_VAL_TP
          WHEN '1' THEN
               CASE PSV_COL_CD
                    ...
                    WHEN '08' THEN
                        CASE WHEN PSV_CT_TP = 'CV'   AND PSV_MODE = 'G' THEN
                                  ls_col_cd :=  'SUM(SUM(A.BILL_CNT + A.R_BILL_CNT)) OVER(PARTITION BY ' || PSV_PB || ') ' ;
                                  ls_alias  :=  'SUM_BILL_CNT';
                             WHEN PSV_CT_TP = 'CV'   AND PSV_MODE = 'N' THEN
                                  ls_col_cd :=  'SUM(A.BILL_CNT + A.R_BILL_CNT) OVER(PARTITION BY ' || PSV_PB || ')';
                                  ls_alias  :=   'SUM_BILL_CNT';
                             WHEN PSV_MODE = 'G' THEN
                                  ls_col_cd :=  'SUM(A.BILL_CNT + A.R_BILL_CNT) ';
                                  ls_alias  := 'BILL_CNT';
                             ELSE
                                  ls_col_cd :=  '(A.BILL_CNT + A.R_BILL_CNT) ';
                                  ls_alias  := 'BILL_CNT';
                        END CASE ;
                    ...
                    WHEN '09' THEN
                        CASE WHEN PSV_CT_TP = 'CV'   AND PSV_MODE = 'G' THEN
                                  ls_col_cd :=  ' CASE WHEN SUM(SUM(A.BILL_CNT + A.R_BILL_CNT)) OVER(PARTITION BY ' || PSV_PB || ') = 0 THEN '
                                            ||  ' NULL ELSE '
                                            ||  '  ROUND(SUM(SUM(A.GRD_AMT)) OVER(PARTITION BY ' || PSV_PB || ') '
                                            ||  ' / SUM(SUM(A.BILL_CNT + A.R_BILL_CNT)) OVER(PARTITION BY ' || PSV_PB || ')) END ';
                                  ls_alias  := 'SUM_BILL_AMT';
                             WHEN PSV_CT_TP = 'CV'   AND PSV_MODE = 'N' THEN
                                  ls_col_cd :=  ' CASE WHEN SUM(A.BILL_CNT + A.R_BILL_CNT) OVER(PARTITION BY ' || PSV_PB || ') = 0 THEN '
                                            ||  ' NULL ELSE '
                                            ||  ' ROUND(SUM(A.GRD_AMT) OVER( PARTITION BY ' || PSV_PB || ') '
                                            ||  ' / SUM(A.BILL_CNT + A.R_BILL_CNT) OVER(PARTITION BY  ' || PSV_PB || ')) END ';
                                  ls_alias  :=  'SUM_BILL_AMT';
                             WHEN PSV_MODE = 'G' THEN
                                  ls_col_cd :=  ' CASE WHEN SUM(A.BILL_CNT + A.R_BILL_CNT) = 0 THEN '
                                            ||  ' NULL ELSE '
                                            ||  ' ROUND(SUM(A.GRD_AMT) / SUM(A.BILL_CNT  + A.R_BILL_CNT)) END ';
                                  ls_alias  :=  'BILL_AMT';
                             ELSE
                                  ls_col_cd :=  ' CASE WHEN (A.BILL_CNT + A.R_BILL_CNT) = 0 THEN '
                                            ||  ' NULL ELSE '
                                            ||  ' ROUND(A.GRD_AMT / (A.BILL_CNT  + A.R_BILL_CNT)) END ' ;
                                  ls_alias  :=  'BILL_AMT';
   ...
   BEGIN
       CASE PSV_COL_CD
            ...
            WHEN '26' THEN
                ls_col_cd :=  'NVL(C.D_CLASS_CD, ''999999999'')';
            WHEN '27' THEN
                ls_col_cd :=  'I.ORD_SALE_DIV ';
            WHEN '31' THEN
                ls_col_cd :=  'S.STOR_TG ';
            WHEN '32' THEN
  ...
  PROCEDURE RPT_PARA
   ...
   BEGIN
        ...
        ls_sql_store := ' S_STORE AS ( '
                      || ' SELECT S.COMP_CD,S.BRAND_CD,B.BRAND_NM,' -- ASP
                      || ' S.STOR_TP,CM1.CODE_NM STOR_TP_NM,'
                      || ' S.STOR_TG,CM6.CODE_NM STOR_TG_NM,'
                      || ' S.STOR_CD,S.STOR_NM, '
                      ...
                      || ' S.SV_USER_ID,NVL(U.USER_NM,FC_GET_WORDPACK(''' || PSV_COMP || ''',''' || PSV_LANG_CD || ''',''UNCLASSIFY'')) SV_USER_NM,'
                      ...
                      || ' S.APP_DIV,CM8.CODE_NM APP_DIV_NM,'
                      || ' S.BUSI_NO,S.REP_STOR_CD,S.TABLE_NO,S.SEAT,S.OPEN_DT,S.CLOSE_DT,S.USE_YN'
                      || ' FROM'
                      || ' ('
                      || ' SELECT'
                      || ' S.COMP_CD,S.BRAND_CD,S.STOR_TP,S.STOR_TG,S.STOR_CD,NVL(L.STOR_NM,S.STOR_NM) STOR_NM,' -- ASP
                      || ' NVL(S.DEPT_CD,''9999999999'') DEPT_CD,NVL(S.TEAM_CD,''9999999999'') TEAM_CD,NVL(S.SV_USER_ID,''9999999999'') SV_USER_ID,'
                      || ' NVL(S.SIDO_CD,''999'') SIDO_CD,NVL(S.REGION_CD,''999999'') REGION_CD,NVL(S.TRAD_AREA,''99'') TRAD_AREA,'
                      || ' S.APP_DIV,S.BUSI_NO,S.REP_STOR_CD,S.TABLE_NO,S.SEAT,S.OPEN_DT,S.CLOSE_DT,S.USE_YN'
        ...
        */
        ...
        -- CM6:상권대분류(30041), CM8:상권소분류(30043)
        ...
        ls_sql_store := ls_sql_store
                      ...
                      || ''' ) CM1,'
                      || ' ( '
                      || ' SELECT R.REGION_CD CODE_CD,NVL(L.LANG_NM,R.REGION_NM) CODE_NM'
                      || ' FROM REGION R, LANG_TABLE L'
                      || ' WHERE L.COMP_CD(+)=R.COMP_CD'   -- ASP
         ...
         ls_sql_store := ls_sql_store
                      ...
                      || ' AND S.STOR_TG=CM6.CODE_CD(+)'
                      || ' AND S.APP_DIV=CM8.CODE_CD(+)'
         ...
         ls_sql_item  := ' S_ITEM AS ('
                      ...
                      || ' I.ITEM_NM,I.STANDARD,I.SALE_START_DT,I.SALE_CLOSE_DT,I.L_CLASS_CD,I.M_CLASS_CD,I.S_CLASS_CD,I.D_CLASS_CD,'
                      ...
                      || ' NVL(IC4.D_CLASS_NM,FC_GET_WORDPACK(''' || PSV_COMP || ''',''' || PSV_LANG_CD || ''',''UNCLASSIFY'')) D_CLASS_NM,' -- ASP
                      || ' IC1.SORT_ORDER L_SORT_ORDER,IC2.SORT_ORDER M_SORT_ORDER,IC3.SORT_ORDER S_SORT_ORDER,IC4.SORT_ORDER D_SORT_ORDER,'
                      || ' I.ORD_UNIT,IC6.ORD_UNIT_NM,'
                      || ' I.SALE_UNIT,IC7.SALE_UNIT_NM,'
                      || ' I.STOCK_UNIT,IC8.STOCK_UNIT_NM,'
                      || ' I.DO_UNIT,IC9.DO_UNIT_NM,I.WEIGHT_UNIT,I.VENDOR_CD,I.SET_DIV,I.SEASON_DIV,ICA.SEASON_DIV_NM,I.CUST_STD_CNT,'
                      || ' I.COST_VAT_YN,I.COST_VAT_RULE,I.COST_VAT_RATE'
                      || ' FROM'
                      || ' ('
                      || ' SELECT I.COMP_CD,I.BRAND_CD,I.ITEM_CD,I.REP_ITEM_CD,I.SALE_PRC,' -- ASP
                      || ' NVL(L.ITEM_NM,I.ITEM_NM) ITEM_NM,I.STANDARD,I.SALE_START_DT,I.SALE_CLOSE_DT,'
                      || ' NVL(C.L_CLASS_CD,''999'') L_CLASS_CD,NVL(C.M_CLASS_CD,''99999'') M_CLASS_CD,NVL(C.S_CLASS_CD,''9999999'') S_CLASS_CD,NVL(C.D_CLASS_CD,''999999999'') D_CLASS_CD,'
                      || ' I.ORD_UNIT,I.SALE_UNIT,I.STOCK_UNIT,I.DO_UNIT,I.WEIGHT_UNIT,I.VENDOR_CD,I.SET_DIV,I.SEASON_DIV,NVL(I.CUST_STD_CNT,0) CUST_STD_CNT,'
                      || ' I.COST_VAT_YN,I.COST_VAT_RULE,I.COST_VAT_RATE'
        ...
        */
        ...
        ls_sql_item := ls_sql_item
                      ...
                      || PSV_LANG_CD
                      || ''' ) IC3,'
                      || ' (SELECT C1.L_CLASS_CD,C1.M_CLASS_CD,C1.S_CLASS_CD,C1.D_CLASS_CD,NVL(C2.LANG_NM,C1.D_CLASS_NM) D_CLASS_NM,C1.SORT_ORDER'
                      || ' FROM ITEM_D_CLASS C1,LANG_TABLE C2'
                      || ' WHERE C1.COMP_CD=C2.COMP_CD(+)'
                      || ' AND C1.COMP_CD='''
                      || PSV_COMP
                      || ''' AND C1.ORG_CLASS_CD='''
                      || ls_org_class
                      || ''' AND C1.COMP_CD||C1.ORG_CLASS_CD||C1.L_CLASS_CD||C1.M_CLASS_CD||C1.S_CLASS_CD||C1.D_CLASS_CD=C2.PK_COL(+)'
                      || ' AND C2.TABLE_NM(+)=''ITEM_D_CLASS'''
                      || ' AND C2.LANGUAGE_TP(+)='''
                      || PSV_LANG_CD
                      || ''' ) IC4,'
                      ...
                      || PSV_LANG_CD
                      || ''' ) IC9,'
                      || ' (SELECT C.CODE_CD SEASON_DIV,NVL(L.CODE_NM,C.CODE_NM) SEASON_DIV_NM'
                      || ' FROM COMMON C,LANG_COMMON L'
                      || ' WHERE C.COMP_CD=L.COMP_CD(+)'
                      || ' AND C.CODE_TP=L.CODE_TP(+)'
                      || ' AND C.CODE_CD=L.CODE_CD(+)'
                      ||q'[ AND C.CODE_TP='02030']'
                      ||q'[ AND C.USE_YN='Y']'
                      || ' AND C.COMP_CD='''
                      || PSV_COMP
                      || ''' AND L.LANGUAGE_TP(+)='''
         ...
         ls_sql_item := ls_sql_item
                      ...
                      || ' AND I.L_CLASS_CD=IC4.L_CLASS_CD(+)'
                      || ' AND I.M_CLASS_CD=IC4.M_CLASS_CD(+)'
                      || ' AND I.S_CLASS_CD=IC4.S_CLASS_CD(+)'
                      || ' AND I.D_CLASS_CD=IC4.D_CLASS_CD(+)'
                      || ' AND I.ORD_UNIT  =IC6.ORD_UNIT(+)'
                      || ' AND I.SALE_UNIT =IC7.SALE_UNIT(+)'
                      || ' AND I.STOCK_UNIT=IC8.STOCK_UNIT(+)'
                      || ' AND I.DO_UNIT   =IC9.DO_UNIT(+)'
                      || ' AND I.SEASON_DIV=ICA.SEASON_DIV(+)';
   ...
   BEGIN
        ...
        IF li_array = 0 THEN
           ...
           ls_sql_sel_up := REPLACE(ls_sql_sel_up, 'SALE_DT', ' SUBSTR(SALE_DT,1,4) || ''-'' || SUBSTR(SALE_DT,5,2) || ''-'' || SUBSTR(SALE_DT,7,2) ||  ''('' || FC_GET_WEEK(''' || PSV_COMP || ''', SALE_DT,  '''|| PSV_LANG_CD || ''') ||  '')''');
