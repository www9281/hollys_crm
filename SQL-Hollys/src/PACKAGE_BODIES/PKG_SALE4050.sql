--------------------------------------------------------
--  DDL for Package Body PKG_SALE4050
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "CRMDEV"."PKG_SALE4050" AS
    ...
    ( 
        ...
        PSV_FILTER      IN  VARCHAR2 ,                -- Search Filter(0:일반판매, 1:상품권(회원권) 판매)
    ...
    ******************************************************************************/
    ...
    BEGIN
        ...
        ||CHR(13)||CHR(10)||Q'[                        WHERE A.COMP_CD  = B.COMP_CD(+) AND  A.BRAND_CD = B.BRAND_CD(+)  AND A.DC_DIV = B.DC_DIV(+) AND  A.COMP_CD  = :PSV_COMP_CD AND A.SALE_DT  = :PSV_SALE_DT ]'
        ||CHR(13)||CHR(10)||Q'[                     ) SDC  ]'
