--------------------------------------------------------
--  DDL for Function GET_M_CLASS_NM
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "CRMDEV"."GET_M_CLASS_NM" 
(
  A_COMP_CD       VARCHAR2
...
BEGIN
  ...
  L_TABLE_NM := 'ITEM_M_CLASS';
  L_COL_NM   := 'M_CLASS_NM';

  SELECT NVL(T2.LANG_NM, T1.M_CLASS_NM)  AS "M_CLASS_NM"
    INTO L_RET_VALUE
    FROM ITEM_M_CLASS T1
        ,LANG_TABLE   T2
   WHERE T1.COMP_CD          = A_COMP_CD
     ...
     AND T2.PK_COL       (+) = T1.COMP_CD||T1.ORG_CLASS_CD || T1.L_CLASS_CD || T1.M_CLASS_CD
  ;
