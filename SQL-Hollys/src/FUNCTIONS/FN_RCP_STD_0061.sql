--------------------------------------------------------
--  DDL for Function FN_RCP_STD_0061
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "CRMDEV"."FN_RCP_STD_0061" (
...
RETURN TBL_RECIPE_BRAND_FOOD AS
    ...
    CURSOR CUR_1 IS
        SELECT  RCP.COMP_CD,
                ...
                CASE WHEN ITC.COST_VAT_YN = 'Y' AND ITC.COST_VAT_RULE = '1' THEN NVL(ICH.COST, ITC.COST) / (1 + ITC.COST_VAT_RATE) / (ITC.ORD_UNIT_QTY * NVL(ITC.WEIGHT_UNIT, 1))
                     ELSE NVL(ICH.COST, ITC.COST) / (ITC.ORD_UNIT_QTY * NVL(ITC.WEIGHT_UNIT, 1)) END         AS UNIT_PRICE,
...
BEGIN
    FOR MYREC IN CUR_1 LOOP
        ...
        ELSE
            nC_ITEM_RCP_COST := ROUND(nC_ITEM_RCP_QTY * MYREC.UNIT_PRICE , 6);
