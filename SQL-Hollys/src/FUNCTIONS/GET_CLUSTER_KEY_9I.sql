--------------------------------------------------------
--  DDL for Function GET_CLUSTER_KEY_9I
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "CRMDEV"."GET_CLUSTER_KEY_9I" 
...
RETURN VARCHAR2 IS
 ...
 FOR CUR_SQL IN ( SELECT X.COLUMN_NAME, 
                  ...
                  AND    X.TABLE_NAME  = UPPER(V_NAME)
	              ) LOOP

    IF (CUR_SQL.COLUMN_POSITION = 1) 
    THEN  KEY_STRING   := KEY_STRING||' CLUSTER (';
    END IF;
